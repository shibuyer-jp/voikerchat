...
対象PR: #10・#11・#12(通知機能一式)・#13(ストリーク修正)・#17(言語切替時の再スケジュール)・#19(daily_limit日次リセット修正)・#24(お気に入りハートボタン削除)・#25(最近使ったシーン)。
...
## 現在の検証状況(未実施・進行中の項目サマリ、2026-07-26時点)

- **daily_limit 日次リセット検証(STEP 3・PART B)**: 未実施。人工的な再現(`last_reset_utc`を2日前にする操作)からやり直しが必要
- **通知の言語検証(STEP 4・Phase B)**: 進行中。端末言語を英語に切替済み。次回の通知配信分で判定待ち
- **STEP 2(Phase A)・STEP 5(Phase C)・Phase D・Phase E**: 未着手
- **STEP 6(シーン選択画面UI確認)**: 未着手。Build 12以降が必要(下記ビルド要件参照)

## ビルド要件(必読)

**本検証には Build 12 以降が必要です。** Build 11(TestFlight配信済み)には STEP 6(シーン選択画面UI確認、
ハートボタン削除・最近使ったシーン)の対象である PR #24・#25 が含まれていません。両PRのマージ時刻
(2026-07-26 16:56〜16:57 JST)が Build 11 のビルド番号更新コミット(2026-07-26 13:09 JST)より後のため、
Build 11 には未収録です。Build 12 以降の TestFlight配信を待ってから STEP 6 を実施してください
(STEP 1〜5・Phase D/Eは Build 11 時点の内容で実施可能です)。

STEP 4(Phase B、アプリ内言語切替時の通知再スケジュール検証)の対象である PR #17 は Build 10 には未収録でしたが、
Build 11 以降には収録済みです(Build 10 のビルド番号更新コミットより後にマージされたため)。
...
| STEP 4: Phase B(言語切替・再スケジュール) | アプリ側(PR #17) | Build 11以降(既に反映済み) |
| STEP 5: Phase C(ストリーク回帰・マイルストーン) | アプリ側(PR #13) | Build 8以降(既に反映済み) |
| STEP 6: シーン選択画面UI確認(ハートボタン削除・最近使ったシーン) | アプリ側(PR #24・#25) | **Build 12以降が必須**(Build 11には未収録) |
...
**実行順序(本編)**: 事前準備 → PART A(daily_limit是正)→ Phase A(通知トグル)→ daily_limit PART B(動作検証・**必須**)→ Phase B(言語切替)→ Phase C(ストリーク回帰)→ STEP 6(シーン選択画面UI確認、Build 12以降)。
本編はここまでで1セッション完結します(STEP 6はBuild 12配信後に追加実施)。Phase D・Eは別途「日をまたぐ・再インストールが必要な項目」を参照してください。
...
## STEP 6: シーン選択画面UI確認(Build 12以降が必須)

PR #24(未完成のお気に入りハートボタン削除)・PR #25(「最近使ったシーン」セクション追加)の実機確認。
自動テストでは見た目・実際の操作感まで検証できないため、以下を目視・実操作で確認する。

| # | 操作 | 期待結果 | NG時に疑うべき箇所 |
|---|---|---|---|
| F1 | シーン選択画面を開き、無料/Premium/ロック済みの各カードを見る | ハートアイコンが表示されていない。ロック済みカードは従来通りロックアイコンが右上に表示される | ハートアイコンが残っている場合、Build 12ではなくBuild 11以前で確認している可能性 |
| F2 | 上記カードの右上余白・整列を確認 | ハート削除前と比べて余白や整列が不自然に崩れていない(隙間ができすぎる・詰まりすぎる等が無い) | `lib/widgets/scene_preview_card.dart`のRow構造(Expandedのみになった右上領域) |
| F3 | シーンを一度も開いていない状態(新規インストール直後、または履歴が無い状態)でシーン選択画面を開く | 「最近使ったシーン」セクションが表示されない | `getRecentSceneIds()`の空リスト判定、`SizedBox.shrink()`分岐 |
| F4 | シーンを1〜3個開き、都度シーン選択画面に戻る | 「最近使ったシーン」に開いた順が新しい順(最新が先頭)で表示される | `recordRecentSceneId()`の並び替えロジック |
| F5 | 同じシーンを複数回開く | 「最近使ったシーン」に同じシーンが重複して表示されない(1件のみ、最新の位置に移動) | 重複統合ロジック(`current.where((id) => id != sceneId)`) |
| F6 | 「続きから」バナーが表示されている状態でシーン選択画面を確認 | バナーに表示中のシーンが「最近使ったシーン」には重複して表示されない(除外されている) | `_getVisibleResumeSceneId()`とフィルタ処理 |
| F7 | 「続きから」バナーが非表示の状態(初回起動時等)でシーン選択画面を確認 | 「最近使ったシーン」が通常どおり最大3件表示される(除外されすぎていない) | バナー非表示判定とフィルタ条件の相互作用 |
| F8 | (Premium解約/ダウングレードのテストが可能な場合)過去にPremiumシーンを開いた履歴がある状態で無料ユーザーとしてシーン選択画面を見る | 「最近使ったシーン」内のPremiumシーンにもロック表示が適用される(閲覧権のバイパスが起きない) | `isLocked: scene.isPremium && !isPremiumUser`の適用漏れ |

- [ ] F1〜F2: ハートボタンが削除され、レイアウト崩れが無い
- [ ] F3: 履歴0件時に「最近使ったシーン」が非表示
- [ ] F4〜F5: 最新順表示・重複統合が正しく動作
- [ ] F6〜F7: 「続きから」バナーとの重複除外が正しく機能し、バナー非表示時は通常表示に戻る
- [ ] F8(実施できる場合): ダウングレード後のロック表示が正しく適用される

**本編(STEP 1〜6)はここまでで完結です。** Phase D・Eは日をまたぐ/再インストールが必要なため、下記の別セクションで実施してください。
...
- [ ] STEP 6(シーン選択画面UI確認、Build 12以降): ハートボタン削除・レイアウト崩れ無し、「最近使ったシーン」の表示/非表示・順序・重複統合・「続きから」バナーとの重複除外が正しく動作
