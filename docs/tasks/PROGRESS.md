...
- [ ] T-35 高品質TTS(3段構成: 端末/広告日解放/プレミアム常時) — 仕様確定 2026-07-16。**着手前に人間の判断待ち(下記参照)**
...
- **T-35 着手保留・要判断**: 下記「判断記録」の T-35 セキュリティ設計の項目を参照し、方針を確認してほしい
- (継続) セキュリティ注意: `C:\Users\takat\Documents\voikerchat` のgit remote URLに
  GitHub PATが平文で埋め込まれている(`https://ghp_...@github.com/...`)。トークンのローテーション
  (再発行・旧トークン失効)と、remote URLをトークン無しの形に直すことを推奨(セッション冒頭のリポジトリ
  探索時に発見。voikerchat本体の作業とは無関係の別クローンだが、認証情報の露出のため報告)
...
- T-35 着手前に発見した設計上の懸念点(詳細は「人間へのお願い」参照): 現行の `grantAdBonus()`(rate_limit_service.dart)は広告視聴報酬をFlutterアプリから直接Supabaseへ書き込む方式で、`usage_logs.ad_reward` イベントは一切記録されていない。RLSも「本人のrate_limits行は本人がUPDATE可」という設計のため、T-35仕様が要求する「サーバー側でusage_logsのad_reward履歴を検証してからクラウドTTSを解放する」対策は、検証対象のログ自体が(現状のRLSでは)クライアントから偽装可能という弱点を持つ。実装を進める前にこの点をユーザーに確認する。
...
- 2026-07-17: T-31 完了。commit `9cd031d`、CI緑(CI/CD - Voikerchat run 29547891381, Flutter CI run 29547891450)。
  T-35 は着手前にセキュリティ設計の確認が必要なため、ユーザーに質問してセッションを一時停止。
  **次アクション: 下記の質問への回答を受けてT-35着手、その後T-36(RUNBOOK指定順序では最後、Master Plan推奨順序にも記載)。**
