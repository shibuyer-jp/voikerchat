...
9. **プレミアムシーンのロック解除バグ修正**(2026-08-06追記、CC実装済み・未マージ)
   ...
   - 検証: 実機で3パターン((i)シーン一覧から購入→開放 (ii)チャット画面から購入→シーン一覧に戻ると開放 (iii)購入済み状態でロック済みシーン再タップ→その場で開放)をアプリ再起動なしで確認すること。**未実施**
   - 担当: 人間(実機検証)。期限: 未定
   - 詳細: `internal-docs/reports/premium_unlock_investigation_20260805.md`(Step1原因調査・(a)/(b)/(c)分類の記録)、PR #53本文参照

10. **オフライン時の挙動**(調査→実装、2026-08-06時点で調査未着手)
    - 現象: 機内モードでアプリを起動すると白画面のまま進まない(2026-08-05、Xiaomi実機で確認)。詳細は本ファイル「改善候補」節の「オフライン起動時に白画面のまま進まない」参照
