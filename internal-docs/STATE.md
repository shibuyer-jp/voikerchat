...
7. **テスターフィードバックの収集** ✅ 完了(収集: 人間、記録: CC、2026-08-06)
   ...
   - 現況: Filipinoテスター1名分を記録(収集日自体は不明、2026-07-31以降)。「1日5問で上限」報告はFREE_DAILY_LIMITが当時5だったための正しい挙動と判明(不具合ではない、TESTER-FEEDBACK.md参照)。「AI応答の日英併記」要望は新規バックログ項目として追加(下記参照)
   - 目的: 本番申請の設問「フィードバックに基づく改善」の回答材料
   - 残作業: 追加テスターからの収集は任意(現状1名分のみ)。期限2026-08-10までに増やすか判断
...
10. **オフライン時の挙動**(調査→実装完了、実機再検証待ち。PR #59・`fix/offline-launch-timeout-and-banner`未マージ)
    ...
    - 修正: FCM/RevenueCat/Supabase呼び出し計6箇所に8秒タイムアウトを追加、独立した初期化を`Future.wait()`でWave 1として並列実行。Supabase初期化が実際に失敗/タイムアウトした場合のみ一度だけオフライン案内バナーを表示
    - 実機検証(1回目、2026-08-06): 白画面解消・シーン選択画面まで到達を確認。通常時(オンライン)の起動速度・Premium状態表示に退行なしを確認。一方で(a)キャッシュ済みセッションがあると通信ゼロで「成功」判定されオフライン案内が出ない (b)機内モードでシーンを開くと生の例外文言が表示される、の2件が新規判明。いずれも修正しPR #59に追加push済み(`auth.getUser()`によるサーバー到達性検証、ネットワークエラーのヒューリスティック判定+`noInternetConnectionError`表示)
    - 担当: CC(実装)完了。人間(実機再検証)待ち。期限: 未定
...
- **AI応答の日英併記(新規検討項目、2026-08-06追記)**: 現状AI応答は日本語のみ。テスターフィードバック(`internal-docs/TESTER-FEEDBACK.md`)のQ2・Q5で2回言及されており、課金しない理由の一つと推測される。内容: AI応答を日本語のみから日英併記に変更する要望。検討事項: (a) 出力トークン増によるコスト影響(現状avg_output=69トークン、単純計算で倍増の可能性) (b) 学習効果の観点(日本語のみで推測させる価値との比較。語学学習アプリとして英語併記が「楽をさせすぎる」可能性も検討要) (c) Premium機能として差別化する選択肢(無料は日本語のみ、Premiumで日英併記等)。担当: 未定。期限: 未定(判断待ち)
