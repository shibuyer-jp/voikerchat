...
6. **年齢設定の不整合を解消** 🔴 **本番アクセス申請前の最優先事項(2026-08-07、iOS購入シートのUNRATED表示発覚により格上げ)**
   ...
   - **項目6は引き続き未完了とする**。検証条件「4箇所が一致していること」のうち、**Google Play「ターゲットユーザーとコンテンツ」の対象年齢層申告(13歳未満を含めていないことの確認)が未確認のまま残るため**。残作業はこの1点に絞られた: Play Console実画面で対象年齢層申告の内容を確認する(`internal-docs/reports/google_play_target_audience_20260807.md`の確認手順を参照)
   - **判断根拠(2026-08-10)**: Google Play(IARC)の「全年齢(3+)」はコンテンツレーティング(暴力表現等の有無を評価する指標)であり、法務文書の対象年齢(利用規約上の年齢制限)とは評価軸が異なるため、両者が異なる数字を示すこと自体は必ずしも不整合ではない。真のリスクは「ターゲットユーザーとコンテンツ」側の申告であり、こちらで13歳未満を含む申告になっている場合はFamilies Policy Requirements(子供向け広告SDKへの切替義務等)が発動し、現行のAdMob(成人向け広告SDK)利用と衝突する。これが未確認のまま残る限り、本番アクセス申請前の最優先事項という位置づけは維持する
