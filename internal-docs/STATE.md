...
- **現在地: 製品版リリース審査中(管理対象の公開オン、公開待ちで停止)**。製品版アクセスは2026-08-18に承認された(申請08-14、想定08-21前後より3日前倒し)。同日、製品版トラックへBuild 22(1.0.0)のリリースを作成し、ストア掲載情報の更新と合わせて計10件を1バッチで審査に送信、審査中。配信国はフィリピン・日本の2か国に限定(理由はDECISIONS.md 2026-08-18参照)。「管理対象の公開」はオンのため、審査通過後も自動公開されず「公開待ち」で停止する。詳細はDECISIONS.md 2026-08-18系列参照。
...
- **iOS**: 1.0.0+20が**App Review承認 → 「Pending Developer Release」到達済み**(2026-08-10)。**Android/iOS同時公開で確定済み**(撤退期限 2026-09-05)。詳細は未完了項目13参照。
- **iOS 1.0.0+22への計測コード反映(①完了・②③未着手)**: Android側はBuild 22で
  計測コード(PR-A/PR-B)を反映済み。iOS側も2026-08-18にios-release.yml
  (use_test_ads=false / main / 50f8e3b)で1.0.0+22をビルドし、App Store
  Connectへのアップロードを完了した(①完了。ITMS-90068警告(MinimumOSVersion
  13.0、2027年春以降は15.0以上必須)を受信したが配信自体は成功しており影響なし)。
  1.0.0+20のPending Developer Releaseはまだ破棄していない(温存)。残る作業は
  ②**成功を確認してから**1.0.0+20のPending Developer Releaseを破棄
  ③1.0.0+22をApp Reviewへ提出、の順。**②を①より先に行わないこと**(ビルド失敗時に
  戻せる承認済み版を失うため)。なお+20のまま公開するか+22に差し替えるかは
  Androidの審査進捗を見て別途判断する(バックログ参照)。担当: 人間+CC。
...
**審査待ち期間中にクリアしたいこと**
- App Store Connect へのストア掲載情報の反映 [判定: 2026-08-28][種別: 実施]
  Google Play側は2026-08-18に反映済みだが、App Store Connect側は未着手。
  唯一の正は `internal-docs/Store-Listing-Copy-v1.5.md`。
  プロモーションテキスト・説明文・サブタイトル・キーワードの4項目。
  iOSの公開前に必ず反映すること。
- 公開日のレビュー依頼準備 [判定: 2026-08-28][種別: 実施]
  配偶者経由の実テスター(16名)へ、公開日にレビュー投稿を依頼できるよう事前に声をかけておく。公開日確定後に実施。`internal-docs/GROWTH_PLAN.md`参照。

**運用・技術(公開前後)**
- iOS 1.0.0を+20のまま公開するか+22に差し替えるかの判断 [判定: 2026-08-28][種別: 判断]
  Androidの審査進捗を見て決定する。撤退期限 2026-09-05。
...
- 音声のPrivacy開示整合 [判定: 2026-08-28][種別: 棚卸し]
  App Store Connectのプライバシー申告に音声データ送信が反映されているか確認する(NSSpeechRecognitionUsageDescription対応済み、申告のみ要確認)。
