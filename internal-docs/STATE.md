...
5. **本番環境へのアクセス申請**
   ...
   - **残る前提作業は1点のみ**: Part 1②の回答本文はテスター自身の購入実績が「確認できていない」という現時点の事実で作成済みだが、提出前にPlay Console → 収益化 → 収益レポート/注文管理で実際の購入実績有無を確認すること(担当: 人間)。確認結果により本文の差し替えが必要になる場合がある
   - クローズドテスト完走(2026-08-14前後見込み)を待って提出
