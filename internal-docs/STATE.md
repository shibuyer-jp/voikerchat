...
- **iOS 1.0.0+22への計測コード反映(未着手)**: Android側はBuild 22で
  ...
  - **リスク**: +20破棄後に+22がリジェクトされると、iOSは承認済みビルドを失う。
    過去2回のリジェクト実績(Guidelines 5.1.1/5.1.2系)を踏まえ、+22は**計測コード追加のみ**に
    留め、UI・課金・プライバシー関連の変更を混ぜない(リジェクト要因を増やさない)。
  - **代替案(未採用、判断材料として保持)**: +20をそのまま公開し、+22は公開後の
    アップデートとして配信する。iOS初期ユーザーの計測データは失うが、承認済みビルドを
    手放さずに済む。撤退期限2026-09-05に間に合わない見込みが立った時点で再検討する。
