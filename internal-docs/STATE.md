...
下記「運用ルール」によりPlay Consoleのトラック設定を変更できないため、着手は製品版アクセス審査の結果が出るまで待つ。書式はCLAUDE.md「バックログ項目の書式ルール」参照(`[判定: YYYY-MM-DD][種別: 実施|判断|棚卸し]`、担当フィールドは書かない)。判定日は公開日2026-08-28仮置きに基づく(冒頭「運用ルール」参照)。詳細な経緯はDECISIONS.mdまたは`internal-docs/reports/`配下の個別レポートを参照。**本ファイルがやることリストの唯一の置き場**(`internal-docs/INDEX.md`参照)。

**審査待ち期間中にクリアしたいこと**
- ストア掲載情報の見直し(ASO)の反映 [判定: 2026-08-28][種別: 実施]
  文言確定は完了(2026-08-17、`internal-docs/Store-Listing-Copy-v1.5.md`が唯一の正)。ストアへの反映(Play Console/App Store Connectへの実入力)は製品版アクセス申請の審査完了後に実施する。`internal-docs/GROWTH_PLAN.md`「反映タイミング」参照。下記「PR #20ストアコピペ反映」と同一作業として1回で実施する。
...
- PR #20ストアコピペ反映 [判定: 2026-08-28][種別: 実施]
  Google Play Console/App Store Connectの該当欄へ反映。上記「ストア掲載情報の見直し」と同一作業(重複管理ではなく同じタスクの別記載)。
