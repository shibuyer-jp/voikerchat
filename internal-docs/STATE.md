...
- **現在地: Android/iOSとも公開待ち。iOSのApp Review結果が公開日を決める**。
  Androidの製品版リリースは2026-08-18に即日審査通過し「公開準備完了」へ到達、
  管理対象の公開がオンのため自動公開されず公開待ちで停止中。iOSは承認済みだった
  1.0.0+20のリリースをキャンセルし、計測コード入りの1.0.0+22を同日15:07に
  App Reviewへ提出した(ステータス: Waiting for Review、Appleの案内では最大48時間)。
  審査結果が届いたときの対応は`internal-docs/TRIGGERS.md`「iOS App Review の結果が
  届いたら」参照。詳細はDECISIONS.md 2026-08-18系列参照。
...
13. **iOSの公開タイミング判断** ✅ 方針確定(2026-08-14)
    - 状態: 1.0.0+20のPending Developer Releaseはキャンセル済み。計測コード入りの
      1.0.0+22を2026-08-18 15:07にApp Reviewへ提出済み(Waiting for Review)。
      **公開対象は +22**(「進行中」の該当ブロック参照)
