...
> 最終更新: 2026-08-10(続4)(RevenueCat RTDN接続が完了。Topic ID `projects/voikerchat/topics/Play-Store-Notifications`、Last received 2026-08-10 4:04 a.m. UTC。バックログ「RevenueCat Android有効化」を全完了へクローズ。事前調査レポートの手順想定が実態と異なっていたため訂正(必要IAMロールはPub/Sub管理者、Pub/Subトピック作成・発行者権限付与はRevenueCatが自動実行)。並行して`api/revenuecat-webhook.ts`のコードレビューで、返金(`CANCELLATION` cancel_reason=`CUSTOMER_SUPPORT`)がPremium剥奪処理から漏れる不具合を発見。2026-06決定(CANCELLATIONでは降格しない)を返金ケースに限り例外化する修正をPR #89として作成したが、Build 21審査中のためマージはBuild 22以降へ保留。RevenueCat上に開発者本人の8/6ライセンステスト購読の記録が見当たらない件も要注意事項として記録(原因[未確認]、実害無し)。詳細はDECISIONS.md 2026-08-10(続4)参照)
> 旧: 2026-08-10(続3)(未完了項目16「開発者本人の購読の扱い」を完了へクローズ。Google Playからの購入確認メール実物で`takatoh01@gmail.com`が既にライセンステストに登録済み・実課金は一切発生していないことを確認した(注文番号・「テスト:」接頭辞・「¥2,120/5 分」の短縮更新間隔が根拠)。未完了項目7の残作業(配偶者経由のBuild 18版フィリピン側再収集)もクローズ、Takatohの判断により再収集は実施せず既存分(2-1・2-2)で申請設問に回答する方針を確定。**[要指摘]** Takatoh判断待ちだった項目(未完了項目6・16・7残作業)はこれで全て解消したが、未完了項目4「統計情報の共有」は業者向けの別コミットメントとして引き続きブロック中であり、「申請前にクリアすべき項目が全て解消」という表現は誤解を招くため、STATE.md内に区別を明記した。詳細はDECISIONS.md 2026-08-10(続3)参照)
> 旧: 2026-08-10(続2)(未完了項目6「年齢設定の不整合」を**完了**へクローズ。Google Play「ターゲットユーザーとコンテンツ」の対象年齢層申告を実画面で確認したところ18歳以上のまま(懸念していた13歳未満混入は無し)で、「未成年ユーザーの使用を制限する」設定が有効なため未成年による定期購入の新規登録・更新もGoogleがブロックしている状態と判明。Takatohはこの設定を変更せず据え置く判断をした。「4箇所が一致した」のではなく「4箇所の実態を確定し、残る差分を意図的な仕様として許容する」形でのクローズであり、検証条件の文言もこれに合わせて訂正した。これにより製品版アクセス申請のブロッカーは解消(`internal-docs/PRODUCTION_ACCESS.md`項目4b更新済み)。詳細はDECISIONS.md 2026-08-10(続2)参照)
> 旧: 2026-08-10(続)(PR #80(法務文書の対象年齢18歳以上→13歳以上への改訂)をTakatohの承認によりマージ、Vercel本番デプロイ後に4URL全て13歳以上表記へ差し替わったこと・未成年者の課金には保護者同意を要する条項の新設を実画面で確認した。ただし未完了項目6は完了にしない(Google Play「ターゲットユーザーとコンテンツ」の対象年齢層申告が未確認のまま残るため、残作業をこの1点に絞った)。Android Build 21のAABビルド・Play Consoleアップロード・審査送信まで完了(テスターへの配信完了は未確認)、業者依頼「アップデート2〜3回」はBuild 16/18/21で充足。PR #80レビュー過程で発見した2件の既存不備(プライバシーポリシーの節番号欠番、ANDROID_RELEASE.mdのキー種別記載不足)を記録・後者は即修正。詳細はDECISIONS.md 2026-08-10(続)参照)
> 旧: 2026-08-10(1.0.0+20がApp Review承認され「Pending Developer Release」に到達(未完了項目13)。方針どおりリリースは保留、Androidと同時公開まで待機。Androidクローズドテストは10日目・完走見込み8/14前後で変更なし。業者依頼「テスト中にアップデートを2〜3回」はBuild 16・18で下限充足済みだが、PR #69(Premium状態不整合の修正)をAndroidへ反映するためBuild 21を3回目として配信することを決定、`pubspec.yaml`を1.0.0+21へ更新しPR #84でマージ済み(タスクA)。統計情報の共有(未完了項目4)はインストール数のみ8/7分が生成され、アクティブデバイス数・DAUは引き続き取得不能(DAUはGoogle側の障害告知による)。業者へ現状報告済み。Play Console「テストのフィードバック」からBuild 18/13時点の評価4件を新規収集(`internal-docs/TESTER-FEEDBACK.md` 2-2)、開発者返信済み。ただし配偶者経由のBuild 18版フィリピン側再収集(未完了項目7残作業)の代替にはならず未回収のまま。詳細はDECISIONS.md 2026-08-10参照)
> 旧: 2026-08-07(続)(未完了項目14「`REVENUECAT_ANDROID_KEY`投入確認」が完了と判明: Build 18に投入済みで、開発者本人が実購入により課金フロー全体〈購入→entitlement反映→シーン解放〉を検証済み。バックログ「RevenueCat Android有効化」も④まで全完了(RTDN接続のみ残)。この検証を受け`internal-docs/PRODUCTION_ACCESS.md`のチェックリスト項目9・Part 1②を訂正。新たなリスクとして開発者本人の購入がライセンステスト扱いか未確認(未完了項目16新設)。iOSの公開タイミング(未完了項目13)は「Android完走を待って同時公開」の方針としたが、製品版アクセス申請の審査(通常7日以内)を踏まえるとAndroid一般公開は8/21〜25頃で、8/14での同時公開は成立しない旨を追記(先行公開/待機のどちらにするかは未確定のまま)。iOS Sandbox課金検証(未完了項目12)をiOS公開前必須へ格上げ(AndroidでのStoreKit外経路の検証だけではPR #53の修正がiOSで有効か保証できないため)。詳細はDECISIONS.md 2026-08-07(続)参照)
...
- **Androidクローズドテスト**: 14日タイマー進行中(起算 2026-07-30 19:31 JST、**2026-08-10時点で10日目**)。Play Console表示「12人のテスターが10日間連続でオプトインしています」[確認済 2026-08-10、Play Console実画面]。完走見込みは従来どおり**2026-08-14前後で変更なし**。**Build 21(versionCode 21)のAABビルド・Play Consoleクローズドテスト(Alphaトラック)へのアップロード・「1件の変更を審査に送信」まで完了**[実施済 2026-08-10、Play Console実画面で「21 (1.0.0) 完全公開を開始」を確認]。現在はGoogleの審査待ちで、**テスターへの配信完了(「選択したテスターに公開されました」表示)はまだ確認できていない**。統計情報の詳細はSTATE.md未完了項目4参照(3指標中2指標が引き続き取得不能)。旧: **2026-08-07時点で8日目**。オプトイン不通問題(SNS内蔵ブラウザ起因)は特定・解消済み。詳細はDECISIONS.md 2026-07-30参照。**運用知見**: Play Consoleの「12人」表示は仕様上の固定表示で実数ではなく、実際は毎日16〜19アカウントが稼働(テスター管理業者〈ココナラ経由〉からの回答、2026-08-04)
- **業者依頼「テスト中にアップデートを2〜3回」の充足状況**(2026-08-10更新): **3回とも実施済みで充足(Build 16 / 18 / 21)**。Build 16(2026-08-03 10:27 JST配信)・Build 18(2026-08-06 13:46 JST配信)は配信完了済み、Build 21は上記のとおり審査送信まで完了(配信完了は未確認)。「運用ルール」節の**新AAB配信では14日タイマーはリセットされない**という既存記載と整合しており、Build 21配信もこの例外規定に含まれる(下記「運用ルール」節参照)
- **iOS**: 1.0.0+20が**App Review承認 → 「Pending Developer Release」に到達(2026-08-10)**[確認済 2026-08-10、App Store Connectからの通知メール。App Version 1.0 / App Apple ID `6789817690` / SKU `voikerchat-ios-001`]。**審査結果待ちの状態は解消**。方針どおり「Release This Version」は押していない。**方針は「Androidの完走を待って同時公開」を維持**、ただし具体的な公開日は未確定(製品版アクセス申請の審査に通常7日以内かかるため、Androidの一般公開自体が2026-08-21〜25頃になる見込み。8/14での同時公開は成立しない。詳細は未完了項目13参照)。**公開まではAdMob関連タスク一式(No Fill再検証・app-ads.txt設置・ストアリンク登録)が着手不可**(バックログ「AdMob公開後タスク」参照)。旧: 1.0.0+20を2026-08-07に再提出、審査結果待ち(承認済みだったbuild 17を「リリースをキャンセル」し、Premium判定不整合の修正(未完了項目17)等を含む+20で再提出。詳細は`internal-docs/IOS_RESUBMISSION_20260807.md`参照)。旧: Build 17(`1.0.0+17`)が2026-08-06に審査承認・手動リリース設定のため未公開。旧: 2026-08-03 14:38 JST再提出・審査待ち、Submission ID `94530390-d70e-4942-b6fc-9c709f735099`。詳細はDECISIONS.md 2026-08-03参照(旧: `1.0.0+15`/commit `7c9687c`、2026-07-29 10:50 JST提出は2回目リジェクトで終了)
...
4. **統計情報の共有(外部サービス保証条件)** 🚧 ブロック中(インストール数のみ8/7分が生成、アクティブデバイス数は未生成、DAUはGoogle側の障害告知により取得不能)
   - 担当: 人間
   - 検証: 出品者へPlay Console統計を共有済みであること
   - 期限: 2026-08-14
   - 現況(2026-08-10): 3指標中1指標しか送れておらず、引き続きブロック中として扱う
     - インストール済みユーザー数: 26(日本23/フィリピン3)。ただし統計データ表に存在する行は2026-08-07の1日分のみで、7/30〜8/6分は生成されていない[確認済 2026-08-10、Play Console実画面]
     - 1日のアクティブなデバイス数: 期間を2026/8/1〜9に指定しても「データを使用できません」[確認済 2026-08-10、Play Console実画面]
     - DAU: Play Console上に「現在、一部の1日のアクティブユーザー(DAU)データをご利用いただけません。Googleは現在、この問題の解決に取り組んでいます」というGoogle公式の告知バナーが表示されている。取得不能[確認済 2026-08-10、Play Console実画面]
   - 対応: 上記現況をテスター管理業者(ココナラ経由)へ報告済み[実施済 2026-08-10]。引き続きアクティブデバイス数・DAUの生成を待つ
   - 旧: 現況(2026-08-07): Play Console「統計情報」は「データを使用できません」表示のままで、共有できる統計が存在しない[確認済 2026-08-07、実画面]。業者回答では反映に約1週間かかり7/30起点で8/6頃から表示見込みだったが、1日以上遅れている
...
6. **年齢設定の不整合を解消** ✅ **完了(2026-08-10)**。「4箇所が一致した」のではなく、**4箇所の実態を確定し、残る差分を意図的な仕様として許容することを決定**してクローズした
   - 担当: 人間
   - 検証(当初): ~~利用規約・プライバシーポリシー・Play Consoleのコンテンツレーティング・App Store Connectのレーティングの4箇所が一致していること~~ → **この条件は達成されていない**(下記表のとおりGoogle Play「ターゲットユーザーとコンテンツ」は18歳以上のまま据え置き)。検証条件を実態に合わせて訂正する: **4箇所それぞれの実態が確定しており、一致しない箇所についてはリスク評価のうえ意図的に許容する判断がTakatohにより行われていること**
   - 期限: 本番申請前(達成)
   ...
   - **[2026-08-10追記]** Takatohの承認によりPR #80をマージした。利用規約(日/英)・プライバシーポリシー(日/英)の4ファイルとも**13歳以上**へ是正され、App Store Connect(13+)と一致した[確認済 2026-08-10、Vercel本番デプロイ後の4URL実画面。詳細はDECISIONS.md 2026-08-10参照]。あわせて利用規約第5条に未成年者の課金には保護者同意を要する旨の条項(日本語版「8. 未成年者による購読」/英語版「Section 8. Subscriptions by minors」)を新設
   - 旧: 項目6は引き続き未完了とする。検証条件「4箇所が一致していること」のうち、Google Play「ターゲットユーザーとコンテンツ」の対象年齢層申告(13歳未満を含めていないことの確認)が未確認のまま残るため。残作業はこの1点に絞られた: Play Console実画面で対象年齢層申告の内容を確認する(`internal-docs/reports/google_play_target_audience_20260807.md`の確認手順を参照)
   - **判断根拠(2026-08-10)**: Google Play(IARC)の「全年齢(3+)」はコンテンツレーティング(暴力表現等の有無を評価する指標)であり、法務文書の対象年齢(利用規約上の年齢制限)とは評価軸が異なるため、両者が異なる数字を示すこと自体は必ずしも不整合ではない。真のリスクは「ターゲットユーザーとコンテンツ」側の申告であり、こちらで13歳未満を含む申告になっている場合はFamilies Policy Requirements(子供向け広告SDKへの切替義務等)が発動し、現行のAdMob(成人向け広告SDK)利用と衝突する
   - **[2026-08-10追記] Google Play「ターゲットユーザーとコンテンツ」の申告内容が判明**: Play Console実画面(ポリシーとプログラム → アプリのコンテンツ)で確認したところ、アプリの対象年齢層は**18歳以上**(最終更新2026年7月14日)だった。懸念していた「13歳未満が含まれている」状態ではなく、Families Policy RequirementsやAdMob利用との衝突は発生していない[確認済 2026-08-10、Play Console実画面]
   - **重要な発見**: 「管理」画面で、**「Googleが未成年と判断したユーザーによるアプリの使用を制限する(省略可)」にチェックが入っている**ことが判明した。このため18歳未満の年齢層(5歳以下/6〜8歳/9〜12歳/13〜15歳/16〜17歳)は選択不可の状態になっている。この設定によりGoogle Playは、未成年と判断したユーザーに対して①Google Playでのアプリの検索・ダウンロード ②アプリ内購入(定期購入の新規登録・既存の定期購入の更新を含む)をブロックしている
   - **Takatohの判断(2026-08-10)**: 現状(18歳以上)のまま変更しない。「変更を破棄」で画面を閉じ、申告内容は変更していない(Play Console側の設定変更は一切なし)。判断理由の詳細はDECISIONS.md 2026-08-10(続2)参照
   - **4箇所の実態(確定、2026-08-10)**:
   ...
   - **差分を許容する根拠**:
     - 規約が許容する範囲より**狭く**配信することはポリシー違反ではない。逆方向(規約より広く配信)であれば問題だが、現状は安全側に倒れている
     - IARCの「全年齢(3+)」はコンテンツレーティング(暴力表現等の有無)であり、法務文書の対象年齢(契約上の利用可能年齢)とは評価軸が異なる
     - PR #80のマージは無駄になっていない。App Store Connect(13+)側で13〜17歳に実際に配信されるため、法務文書を13歳以上へ揃える必要があった
   - **製品版アクセス申請への影響**: 本件のクローズにより、**製品版アクセス申請のブロッカーは解消した**。`internal-docs/PRODUCTION_ACCESS.md`のチェック項目4b(ターゲットユーザーとコンテンツの確認)も確認済みへ更新済み
...
7. **テスターフィードバックの収集** ✅ 完了(収集: 人間、記録: CC、2026-08-06)
   ...
   - 旧: 残作業(予定): Build 18配信後の再収集を2026-08-07夜に配偶者へ依頼予定[本人申告 2026-08-07]。Build 18には無料枠10/日・オンボーディング拡充・辞書機能が含まれるため、Q3「5問で上限」の不満が解消されたかを確認する。回収でき次第TESTER-FEEDBACK.mdへ追記し、PRODUCTION_ACCESS.mdの回答案を最新化すること
   - **[2026-08-10追記]** Play Console「テストのフィードバック」からBuild 18/13時点の評価4件を新たに収集した(`internal-docs/TESTER-FEEDBACK.md` 2-2参照)。ただしこれは上記の残作業(配偶者経由のフィリピン側再収集)の代替にはならない(収集元・質問設計が異なるため)。
   - **残作業のクローズ(2026-08-10)**: **Takatohの判断により、配偶者経由の再収集は実施せず、既存分で対応することを決定した**。理由: 回収の見込みが無いため。製品版アクセス申請の設問には、業者経由のテスターフィードバック4件(2-2、Android 14〜16での動作確認を含む)と、7月に配偶者経由で得た既存分(2-1)で回答する。**残作業は宙に浮いた状態から脱し、クローズとする**
...
13. **iOSの公開タイミング判断**(2026-08-07追記) 🟡 **1.0.0+20が App Review 承認・「Pending Developer Release」に到達(2026-08-10)。公開日程は未確定**
    - **[2026-08-10追記]** 1.0.0+20がApp Review承認され、ステータスが「Pending Developer Release」になった[確認済 2026-08-10、App Store Connectからの通知メール。App Version 1.0 / App Apple ID `6789817690` / SKU `voikerchat-ios-001`]。審査結果待ちの状態は解消
    - 方針どおり「Release This Version」は押していない。**「Androidの完走を待って同時公開」方針は維持**(下記参照)
...
16. **開発者本人の購読の扱い**(2026-08-07追記) ✅ **完了(2026-08-10、リスクは存在しなかったと確認)**
    - 内容: 未完了項目14の課金フロー検証で、開発者本人がPremium定期購入を実際に購入した
    - 旧リスク: Play Console のライセンステストアカウントとして登録されているかが未確認。未登録の実購入であれば、解約しない限り実課金され続ける懸念があった(**旧記載の訂正**: 金額は「毎月$12.99」としていたが、これはApp Store側の価格であり誤り。実際の購入はAndroid〈Google Play〉のため、正しくはJPY 2,120〈2026-08-07の価格改定前の価格〉だった)
    - **確認結果(2026-08-10)**: Google Playからの購入確認メール実物で、`takatoh01@gmail.com`は既にライセンステストに登録済みであり、**実課金は一切発生していないこと**を確認した[確認済 2026-08-10、メール実物]。根拠:
      - 注文番号 `GPA.3366-1618-4848-35600` / 注文日 2026-08-06 15:58:47 JST / アカウント `takatoh01@gmail.com`
      - メール本文に「これはテスト用の定期購入で、このスケジュールに従って繰り返されます。請求が行われることはありません。」と明記
      - アイテム名が「テスト: Voikerchat Premium (Voikerchat)」で、テスト購入を示す「テスト:」接頭辞が付いている
      - 課金周期の表示が「¥2,120/5 分」であり、実際の月額課金ではなくライセンステスト特有の短縮更新間隔(5分毎)になっている
    - **運用知見**: Play Consoleのライセンステスト登録一覧の画面確認より、**Google Playからの購入確認メールを確認するほうが確実**。テスト購入は画面上の挙動が実購入と区別できないため、メールの「請求が行われることはありません」という明記や「テスト:」接頭辞のほうが確実な根拠になる
    - **対応不要**: 解約・返金申請・追加のライセンステスト登録はいずれも不要
    - **副次的な利点**: この購読は更新間隔が5分に短縮されているため、RTDN接続・返金時のPremium剥奪の検証(バックログ参照)に活用できる可能性がある。詳細はROADMAP.md区分3参照
    - 担当: 人間。期限: 速やかに(達成)
...
### 製品版アクセス申請前にクリアすべき項目の状況(2026-08-10時点)

Takatohの判断が必要だった項目(年齢設定の不整合〈未完了項目6〉・ライセンステスト登録の確認〈未完了項目16〉・テスターフィードバック再収集の要否〈未完了項目7残作業〉)は**2026-08-10までにすべて解消**した。

**[要指摘]** 一方、未完了項目4「統計情報の共有(外部サービス保証条件)」は引き続き🚧ブロック中である(3指標中1指標のみ生成)。ただし本項目はGoogleの製品版アクセス申請フォーム自体の設問ではなく、**テスター管理業者への契約上の約束事**(項目名の「外部サービス保証条件」が示すとおり)であり、Google側の申請可否を直接ブロックするものではない。そのため「製品版アクセス申請前にクリアすべきGoogle側の判断待ち項目」という意味では全て解消したと言えるが、「本番運用に向けてクリアすべきこと全般」という意味では統計情報の共有は引き続き未解消である。この区別を曖昧にしないよう、ROADMAP.md区分2でも統計情報の共有は完了扱いにしていない(下記参照)。

残るのは2026-08-14前後のクローズドテスト完走待ち、`PRODUCTION_ACCESS.md`のチェックリスト全項目の最終確認、統計情報の共有(業者向け、進行中)である。

## バックログ(テスト完走後)

旧「次タスク」の未完了分。下記「運用ルール」によりPlay Consoleのトラック設定を変更できないため、着手はテスト完走(2026-08-14目安)後。

- **RevenueCat Android有効化** ✅ **完了(2026-08-10、①②③④+RTDN接続すべて完了)**: ①Google Play Consoleで定期購入商品を作成 ✅完了(2026-07-29、`voikerchat_premium_monthly:monthly-autorenew`、174か国) → ②RevenueCatにAndroidアプリ登録+Google Playサービスアカウント認証 ✅完了(2026-08-04)[本人報告、Claude Code未検証]。Google Cloudプロジェクト`voikerchat`(Firebaseと同一)でPlay Android Developer API/Play Developer Reporting API/Cloud Pub/Sub APIを有効化、サービスアカウント`revenuecat@voikerchat.iam.gserviceaccount.com`を作成しPlay Consoleへ招待(アプリ情報閲覧/売上データ/注文と定期購入の管理)。RevenueCatにPlay StoreアプリをApp ID `appf7acdb482b`で追加、Productインポート成功(Published) → ③Offering/Productをマッピング ✅完了(2026-08-04)。Entitlement `Premium`にAttach(App Store版と併存)、Offering `default`の`$rc_monthly`に追加。**Entitlement識別子の大小文字厳密一致は結果的に`Premium`で確定** → ④ビルドに`REVENUECAT_ANDROID_KEY`を渡す ✅完了(2026-08-07確認)。Build 18に投入済みであることを、開発者本人の実購入による課金フロー全体の検証(未完了項目14参照)で確認した → **⑤RTDN(Real-Time Developer Notifications)接続 ✅完了(2026-08-10)**[確認済 2026-08-10、RevenueCatダッシュボード実画面]。Topic ID `projects/voikerchat/topics/Play-Store-Notifications`、ステータス「Connected to Google」(緑チェック)、Last received: 2026-08-10 4:04 a.m. UTC(Play Consoleからのテスト通知を受信)。
  - **実施手順が事前調査(`internal-docs/reports/revenuecat_rtdn_investigation_20260807.md`)の想定と異なっていた**(2026-08-10、レポート本文を訂正済み): 必要なIAMロールは「Pub/Sub編集者」では不足し**「Pub/Sub管理者」(`roles/pubsub.admin`)が必要**だった(「Your Google service account credentials do not have permission to create a Google Cloud Pub/Sub topic.」エラーで発覚)。Pub/Subトピックの作成とGoogle管理システムサービスアカウント(`google-play-developer-notifications@system.gserviceaccount.com`)への発行者権限付与は、いずれも人間の手動作業ではなく**RevenueCatの「Connect to Google」操作が自動実行**した。サービスアカウント`revenuecat@voikerchat.iam.gserviceaccount.com`のロールは現在「Pub/Sub管理者/Pub/Sub編集者/モニタリング閲覧者」の3つ
  - **運用上の落とし穴**: GCPのロール検索で「Pub/Sub Lite 管理者」が先に表示され誤って選択しやすい(別サービスのロールで無効)。正しいのは「Pub/Sub 管理者」
  - 所要時間実測: 約20分(権限エラーの解決を含む)
  - 担当: 人間(ダッシュボード作業)完了
  - **[要注意、2026-08-10]** RevenueCat上に2026-08-06のライセンステスト購読(未完了項目16参照、注文番号`GPA.3366-1618-4848-35600`)が見当たらない。Customers → Active subscribers=0人、Expired=0人、Sandbox=4人(いずれもentitlementsが無く購入イベントも未記録、Sandbox dataトグルをONにして確認済み)[確認済 2026-08-10]。Google Play側では購入が成立しているにもかかわらず、RevenueCat側に該当する購読レコードが見当たらない。**原因は[未確認]**。RTDN未接続だったこと、8/7の実機検証でアプリ削除・再インストールにより匿名user_idが再生成されたこと、ライセンステスト購読が短期間で終了したこと等がcandidateだが特定していない。**実害の評価**: 開発者本人のテスト購読1件が追跡できないのみで、実ユーザー・収益への影響は無い。RTDN接続済みの現在は今後の購入が追跡される見込み。Build 21配信後の実機Paywall確認時に新規購入を行い、RTDN経由のイベント到達を観察する予定
- **返金時のPremium剥奪の調査** 🔧 **調査完了・原因特定済み(2026-08-10)。修正PR作成済み・未マージ**: RevenueCatには独立した`REFUND`イベント種別が存在せず、返金は`CANCELLATION`イベントの`cancel_reason='CUSTOMER_SUPPORT'`として届くことをRevenueCat公式ドキュメント「Event Types and Fields」で確認した(2026-08-10)。現状の`api/revenuecat-webhook.ts`は`REVOKE_EVENT_TYPES=EXPIRATION`のみで`CANCELLATION`を一律`action='none'`として素通りしており、**返金が成立してもPremiumが剥奪されない不具合**があった。`EXPIRATION`に頼ることもできない(返金後も自動更新設定が有効なままのことがあり、その場合はRENEWALが先に発火しEXPIRATIONが届かない)。2026-06の既存決定「CANCELLATIONでは降格しない。EXPIRATIONのみ降格」は自発的解約(UNSUBSCRIBE)については引き続き正しく、本件は**その決定の全面撤回ではなく返金ケースに限定した例外追加**。修正はPR #89(`fix/refund-cancellation-premium-revoke-20260810`)として作成済みだが、**Android Build 21が審査中のためBuild 22以降にまとめて反映する方針でマージは保留**。実際の返金イベントによる動作検証は未実施(検証可能な生きた購読が現時点で存在しないため)。詳細はDECISIONS.md 2026-08-10(続4)参照。担当: CC(実装)完了、人間(PRレビュー・マージ判断)は未定
...
- Build 16(2026-08-03配信)・Build 18(2026-08-06配信)・Build 21(2026-08-10審査送信、配信完了は未確認)のリリースのみ例外。**新AAB配信では14日タイマーはリセットされない**ため、今後もビルド配信自体は可能
...
### [優先: 低] プライバシーポリシーの節番号が欠番(2026-08-10、PR #80レビュー過程で発見)

- **不具合**: 日本語版・英語版とも、1〜11のあと12を飛ばして13(日本語版「お問い合わせ窓口」/英語版「13. Language」)へ続いている。さらに日本語版には英語版の「13. Language」に相当する節が存在せず、日英で構成が一致していない
- 影響: 軽微(表示上の番号飛びのみ)。ただし他文書(利用規約側の相互参照等)からの参照時に誤りを誘発しうる
- 対応方針: **今回は修正しない**。法務文書の改訂は都度Takatohの承認が必要なため、記録のみに留める
- 担当: 未定。期限: 未定(次回の法務文書改訂時にあわせて解消を検討)
