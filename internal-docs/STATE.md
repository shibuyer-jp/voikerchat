...
- **iOS**: Build 17(`1.0.0+17`)が2026-08-06に**審査承認**。ただしリリース方式を**手動リリースへ変更済みのため、App Storeには未公開**[本人報告 2026-08-07]。**公開タイミングの判断が未決**(Androidの完走 2026-08-14 と揃えるか、先行して公開するか)。**公開まではAdMob関連タスク一式(No Fill再検証・app-ads.txt設置・ストアリンク登録)が着手不可**(バックログ「AdMob公開後タスク」参照)。旧: 2026-08-03 14:38 JST再提出・審査待ち、Submission ID `94530390-d70e-4942-b6fc-9c709f735099`。詳細はDECISIONS.md 2026-08-03参照(旧: `1.0.0+15`/commit `7c9687c`、2026-07-29 10:50 JST提出は2回目リジェクトで終了)
...
13. **iOSの公開タイミング判断**(2026-08-07追記) 🔴 **要判断**
    - 状況: Build 17は2026-08-06に審査承認済みだが、手動リリース設定のため未公開
    - 判断が必要な点: (a) すぐ公開する / (b) Androidの完走(2026-08-14前後)を待って同時公開する
    - (a)のメリット: AdMob関連タスク一式(No Fill再検証・app-ads.txt設置・ストアリンク登録)に着手できる。No Fillの原因が「App Store未公開」と判断されている(DECISIONS.md 2026-07-26)ため、公開しない限り検証が進まない
    - (a)のリスク: 公開後は一般ユーザーが流入し、不具合が実ユーザーに影響する。Android側はまだBuild 18で検証中
    - 担当: 人間(判断)。期限: 判断のみなら随時

14. **Build 18への`REVENUECAT_ANDROID_KEY`投入有無の確認**(2026-08-07追記)
    - 内容: Build 18(2026-08-06配信)のビルド時に`--dart-define=REVENUECAT_ANDROID_KEY=<実キー>`を渡したかが未記録
    - 影響: 渡していればAndroidの購読ボタンが有効化されている。製品版アクセス申請のPart 1②で「テスターは課金・購読フローを一度も体験していない」と開示する予定のため、事実と異なると申請内容の誤りになる
