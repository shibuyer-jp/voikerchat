...
- **revenuecat-webhookの監査ログ欠落** → **PR作成済み・マージ待ち(2026-08-13、R2)**: `logWebhookEvent()`を新設し、`usage_logs.event`のCHECK制約(8値固定)を変更せずに既存の`premium_sync`を再利用、`metadata.source`で`'webhook'`(webhook側)/`'premium-sync'`(premium-sync.ts側)を区別する設計で実装した。action(`none`/`grant`/`revoke`)3経路すべてで記録、ログ書き込み失敗はwebhook本体の処理を止めない(RevenueCatへの200応答をブロックしない)。PRは作成済みだが、**mainへのマージはクローズドテスト完走確認(2026-08-14前後)後に人間が判断**(Vercel自動デプロイでテスター61名使用中の本番APIに即反映されるため)。担当: CC(実装)+人間(マージ判断)
...
- api/ に tsconfig.json / lint 設定が存在しない
  ...
  - **[2026-08-13追記]** R2(webhook監査ログ追加)実装中に手がかりを発見: `api/premium-sync.ts`の5件はいずれも、関数の`supabase`引数を`ReturnType<typeof createClient>`型で受けている箇所(`logSyncAttempt`の呼び出し4箇所+関数内の`.insert()`1箇所)に集中しており、同じ`supabase`変数を`SupabaseClient`型(`@supabase/supabase-js`からimport、`api/_premium.ts`の`upsertPremiumStatus`が使用)で受けている呼び出しはエラーにならない。今回`api/revenuecat-webhook.ts`に新設した`logWebhookEvent()`は当初`logSyncAttempt`と同じ`ReturnType<typeof createClient>`型でコピーし、同種のエラーを新たに3件発生させたが、`SupabaseClient`型に変更したところ解消した(該当PR参照)。`premium-sync.ts`側の5件はR2のスコープ外のため未修正のまま残しているが、**同じ型置き換えで解消する可能性が高い**[未検証、原因の断定はR3で]
