...
- **バックログの判定日は「公開日」を2026-08-28と仮置きして算出している**(2026-08-17時点、実際の公開日は未確定)。公開日 / 公開+2週(09-11) / 公開+1ヶ月(09-28) / 公開+3ヶ月(11-28) の4アンカーを使用。**実際の公開日が確定したら、バックログ全項目の判定日を一括で置換すること**(CLAUDE.md「バックログ項目の書式ルール」参照)

## 進行中

- **現在地: Android/iOSとも公開待ち。iOSの再審査結果が公開日を決める**。
  Androidの製品版リリースは2026-08-18に即日審査通過し「公開準備完了」へ到達、
  管理対象の公開がオンのため自動公開されず公開待ちで停止中(詳細はDECISIONS.md
  2026-08-18系列参照)。iOSは承認済みだった1.0.0+20のリリースをキャンセルし、
  計測コード入りの1.0.0+22を2026-08-18 15:07にApp Reviewへ提出したが、
  2026-08-19、Guideline 2.3.10(Accurate Metadata)でリジェクトされた。指摘は
  「App Store説明文にGoogle Playへの言及がある」の1点のみで、バイナリへの
  指摘はゼロ。2026-08-20、説明文(英語・日本語の両ロケール)から当該記述を
  除去し、Resolution Centerへ返信のうえ、**新規ビルド無しで**メタデータのみ
  再提出した。同日「審査待ち(Waiting for Review)」到達をASC実画面で確認済み。
  対象ビルドは1.0.0(22)のまま変更なし。審査結果が届いたときの対応は
  `internal-docs/TRIGGERS.md`「iOS 1.0.0+22 の再審査結果が届いたとき」参照。
  詳細はDECISIONS.md 2026-08-19系列参照。
...
- **iOS 1.0.0+22への計測コード反映(①②③すべて完了)**: Android側はBuild 22で
  計測コード(PR-A/PR-B)を反映済み。iOS側も2026-08-18に①ios-release.yml
  (use_test_ads=false / main / 50f8e3b)で1.0.0+22をビルドしApp Store Connectへ
  アップロード(ITMS-90068警告(MinimumOSVersion 13.0、2027年春以降は15.0以上
  必須)を受信したが配信自体は成功しており影響なし)②1.0.0+20のPending
  Developer Releaseをキャンセル③1.0.0+22をApp Reviewへ提出(15:07、Waiting
  for Review)まで完了した。ASCの仕様上、アプリ名の変更には承認済みリリースの
  キャンセルが必須だったため②が発生した。+20のまま公開するB案ではなく+22へ
  差し替えるA案を採用した理由(計測データ・ASO・レビュー根回し時間の3点)は
  DECISIONS.md 2026-08-18(続5)参照。
...
**公開前に人間の判断を要する項目はゼロになった**: 5(本番アクセス申請)は2026-08-14に提出完了・2026-08-18に承認済み、12(iOS Sandbox課金検証)はコードレビュー確認で代替済み、13(iOSの公開タイミング)はAndroid/iOS同時公開で確定済み(撤退期限 2026-09-05)。**残るのは製品版リリースの審査待ちのみ**で、審査結果が届いたときの対応は`internal-docs/TRIGGERS.md`「製品版リリースの審査結果が届いたら」参照。これ以外の項目(バックログ・改善候補)はすべて公開後でよい。
...
13. **iOSの公開タイミング判断** ✅ 方針確定(2026-08-14)
    - 状態: 1.0.0+20のPending Developer Releaseはキャンセル済み。1.0.0+22は
      ...
      のみ再提出済み(Waiting for Review)。**公開対象は +22**(「進行中」の
      該当ブロック参照)
...
21. **iOS 1.0.0+22 メタデータ再審査の結果確認**
    - 却下時はResolution Centerの指摘内容を`internal-docs/DECISIONS.md`へ記録する。
    - 担当: 人間。期限: 撤退期限 2026-09-05

## 直近の変更(最新1件のみ。過去分はDECISIONS.md参照)

**2026-08-19〜20**: iOS 1.0.0+22がApp Review Guideline 2.3.10(Accurate
Metadata)でリジェクトされた(2026-08-19、Submission ID
a9d47ca9-399f-4211-b80e-1a4e4505d9d2)。指摘は`internal-docs/Store-Listing-Copy-v1.5.md`
「詳細説明」内の解約方法の記述に「Google Play」への言及がある1点のみで、
バイナリへの指摘はゼロだった。同ファイルの英語版・日本語版の該当箇所を
両ストア共通の中立表現へ書き換え、2026-08-20に**新規ビルド無しで**
メタデータのみ再提出し、同日Waiting for Review到達をASC実画面で確認済み。
適用チェックリストの「年齢設定 13+→18+」行はPR #112の「※要検証」注記を
正としそのまま維持(削除しない)。詳細はDECISIONS.md 2026-08-19系列参照。
...
書式はCLAUDE.md「バックログ項目の書式ルール」参照(`[判定: YYYY-MM-DD][種別: 実施|判断|棚卸し]`、担当フィールドは書かない)。判定日は公開日2026-08-28仮置きに基づく(冒頭「運用ルール」参照)。詳細な経緯はDECISIONS.mdまたは`internal-docs/reports/`配下の個別レポートを参照。**本ファイルがやることリストの唯一の置き場**(`internal-docs/INDEX.md`参照)。

**審査待ち期間中にクリアしたいこと**
- 公開日のレビュー依頼準備 [判定: 2026-08-28][種別: 実施]
  配偶者経由の実テスター(16名)へ、公開日にレビュー投稿を依頼できるよう事前に声をかけておく。公開日確定後に実施。`internal-docs/GROWTH_PLAN.md`参照。

**運用・技術(公開前後)**
- ココナラ取引の評価入力 [判定: 2026-08-27][種別: 実施]
  期限は2026-08-28 00:00で、期限後の入力は一切不可(運営による延長・再入力も不可)。双方が期限内に入力した場合のみ両者の評価が同時公開される。評価コメントはサイト上で公開されるため、アプリ名・パッケージ名等の特定情報は記載せず、匿名で投稿する。
- iOSサブスクリプションの審査ステータス確認 [判定: 2026-08-20][種別: 棚卸し]
  今回の提出は「iOSアプリ1.0」1件のみで、過去の提出時に含まれていたサブスクリプショングループ Premium と Voikerchat Premium Monthly が含まれていなかった。Build 20の審査時に承認済みで変更がないため再審査不要という理解だが未確認。公開前にステータスを確認すること。
- スクリーンショットの英語UI版への差し替え [判定: 2026-08-28][種別: 実施]
  `internal-docs/Store-Listing-Copy-v1.5.md`「スクリーンショット構成」節の7枚構成で撮り直す。公開後の差し替えでも可。
- 年齢表記の不整合解消 [判定: 2026-09-11][種別: 実施]
  ストア説明文「18歳以上を対象」とプライバシーポリシー第8条「13歳以上を対象」が食い違っている。説明文のほうが厳しい主張でありlaunchのブロッカーではないと判断したが、公開後に文言を統一する。
- iOS MinimumOSVersion を 13.0 → 15.0 に引き上げ [判定: 2026-11-28][種別: 判断]
  Apple公式期限は2027年春。着手時期を判定日に判断する。
- 配信国の追加検討 [判定: 2026-09-28][種別: 判断]
  現在フィリピン・日本のみに限定している。EU/EEA追加時はDSA販売者情報の申告が前提となる。追加要否を判定日に判断する。
- 返金分岐の発火監視 [判定: 2026-09-28][種別: 判断]
  `cancel_reason='CUSTOMER_SUPPORT'`が返金を伴わない解約で誤発火していないか、問い合わせ有無を見て対応要否を判断する。
- Push通知Phase2 [判定: 2026-09-28][種別: 実施]
  APNsキーのFirebaseアップロード→PR #14マージ→実機テスト。
- AdMob公開後タスク [判定: 2026-09-11][種別: 実施]
  No Fill再検証・app-ads.txt設置・ストアリンク登録・AdMob準備状況レビュー確認。
- Android署名fail-fast(PR #5、実装済み・マージ保留) [判定: 2026-09-11][種別: 実施]
  Windows Laptopでのローカルgradle検証待ち。
- release_verification_session_20260726.mdの残タスク(STEP2/4/5、Phase D/E) [判定: 2026-08-28][種別: 実施]
  実機での破壊的操作を伴うため、公開のタイミングでまとめて実施する。
- api/*.tsのテスト基盤整備 [判定: 2026-11-28][種別: 判断]
  jest/vitest等のユニットテスト基盤(型チェックはR3で別途解消済み、本項目は別軸)。導入するか判定日に判断する。
- 本番Supabaseの検証用バックアップテーブル削除 [判定: 2026-08-28][種別: 実施]
  `_rate_limits_daily_limit_backup_20260726` / `_rate_limits_verification_backup`を削除する。
- 通知履歴の既存DB行クリーンアップ [判定: 2026-09-28][種別: 実施]
  is_read=true/status='scheduled'の残存3件を削除する(影響軽微)。
- 通知履歴の表示時ローカライズ改修 [判定: 2026-09-28][種別: 判断]
  現状「配信時点の言語で保持」が仕様。ユーザー要望が出ているか判定日に確認し、着手要否を判断する。
- 小タスク: G6ダイアログを権限取得済み時はスキップする改善 [判定: 2026-11-28][種別: 判断]
  任意の改善。着手するか判定日に判断する。
- git stash@{0}〜{4}の整理 [判定: 2026-08-28][種別: 実施]
  移植済み/ビルドノイズのスタッシュをまとめて`git stash drop`する。
- Vercelプロジェクト2重(voikerchat / voikerchat-x621)の整理 [判定: 2026-09-11][種別: 実施]
- RevenueCatダッシュボード未使用Entitlement`Voikerchat Pro`・Offering `$rc_annual`/`$rc_lifetime`の整理 [判定: 2026-09-28][種別: 実施]
- voikerchat.comトップページの訴求文言とストア掲載情報のズレ [判定: 2026-08-28][種別: 棚卸し]
  ズレの有無をまず確認する(2026-08-17時点で未確認)。
- 旧Vercelデプロイ(`*.vercel.app`個別URL)の`internal-docs/`分離前スナップショット露出懸念 [判定: 2026-09-11][種別: 棚卸し]
  プレビュードメインは認証保護済み、個別URLは未確認のため確認する。
- iOS購入シートの年齢表示確認 [判定: 2026-08-28][種別: 棚卸し]
  App Store公開後、実際の購入シートで年齢レーティングが13+として表示されるか確認する(TestFlight環境で「UNRATED」表示を観測したため。App Store Connect側は13+で確定済み)。
- PR #53の実購入時挙動の監視 [判定: 2026-09-11][種別: 判断]
  購入直後にPremiumシーンが即時解除されるか、テスターからの同種報告が無いか見て対応要否を判断する(実機検証を断念しコードレビュー確認に留めた経緯による)。
- バックログ期限切れの自動検出 [判定: 2026-09-28][種別: 実施]
  判定日を過ぎた項目をGitHub Actions(週次cron)が検出しIssueを立てる仕組み。公開前にCIへ手を入れると公開作業をブロックするリスクがあるため、公開後に着手する。

**製品機能(公開後)**
- 年額プラン追加[優先: 高、公開後の収益改善の筆頭] [判定: 2026-08-28][種別: 実施]
...
- 学習スコアの可視化[優先: 高] [判定: 2026-08-28][種別: 実施]
  会話数・連続日数・使用語彙数など、追加AI呼び出し無しで実装できる範囲から着手(トークンコストゼロ)。
- 無料トライアル(全機能開放)[優先: 中] [判定: 2026-09-28][種別: 判断]
  初回3日間など短めから試し実績を見て延長判断。トークンコスト増に注意。運用が落ち着いてから着手可否を判断する。
- ユーザー定義シーン[優先: 中] [判定: 2026-09-28][種別: 判断]
  プロンプト長増によるキャッシュ効率低下、ユーザー入力の不適切設定対策が必須。着手可否を判断する。
- 実務ロールプレイ・シーン追加(役割反転型)[優先: 中] [判定: 2026-09-28][種別: 判断]
...
- 既存シーンの利用実績レビューと整理[優先: 中] [判定: 2026-09-28][種別: 判断]
...
- B(介護事業者)ルートの再調査[優先: 低] [判定: 2026-11-28][種別: 判断]
...
- Google Play対象年齢層の13〜17歳への拡大検討[優先: 低] [判定: 2026-11-28][種別: 判断]
  現行の未成年制限チェックを外す案。外すと保護者同意条項の実効性が失われるリスク・年齢確認画面実装が必要になる可能性あり。着手条件: 学生層の実需要確認後。

**事業横断**
- shibuyer.jp リデザイン(ブランドハブ化) [判定: 2026-11-28][種別: 実施]
...
- SNSショート動画による集客の着手判断 [判定: 2026-09-28][種別: 判断]
...
- Tokyo Bible / KaigoTalkリポジトリのアーカイブ [判定: 2026-09-28][種別: 実施]
...
- shibuyer-opsのオープンIssue整理 [判定: 2026-09-28][種別: 実施]
