...
> クローズドテスト完走(2026-08-14前後見込み)以降とする。
...
**本検証には Build 12 以降が必要です。** Build 11 には STEP 6(シーン選択画面UI確認、ハートボタン削除・最近使った
シーン)の対象である PR #24・#25 が含まれていません。両PRのマージ時刻(2026-07-26 16:56〜16:57 JST)が Build 11 の
ビルド番号更新コミット(同 13:09 JST)より後のため、Build 11 には未収録です(STEP 1〜5・Phase D/E は Build 11
時点の内容で実施可能)。

STEP 4(Phase B、アプリ内言語切替時の通知再スケジュール検証)の対象である PR #17 は Build 10 には未収録でしたが、
Build 11 以降には収録済みです。

> **2026-08-07 追記**: 現在の配信ビルドは Android Build 18(2026-08-06 13:46 JST配信)/ iOS Build 17(2026-08-06
> 審査承認、未公開)であり、**上記のビルド要件はいずれも既に充足している**。本セッションの残タスクを実施する際は
> ビルド要件を気にする必要はない。
...
| STEP 6: シーン選択画面UI確認(ハートボタン削除・最近使ったシーン) | アプリ側(PR #24・#25) | Build 12以降(現行Build 18で反映済み) |
...
**実行順序(本編)**: 事前準備 → PART A(daily_limit是正)→ Phase A(通知トグル)→ daily_limit PART B(動作検証・**必須**)→ Phase B(言語切替)→ Phase C(ストリーク回帰)→ STEP 6(シーン選択画面UI確認)。
...
| # | 操作 | 期待結果 | NG時に疑うべき箇所 |
|---|---|---|---|
| F1 | シーン選択画面を開き、無料/Premium/ロック済みの各カードを見る | ハートアイコンが表示されていない。ロック済みカードは従来通りロックアイコンが右上に表示される | ハートアイコンが残っている場合、Build 12より前のビルドで確認している可能性 |
...
- [ ] STEP 6(シーン選択画面UI確認): ハートボタン削除・レイアウト崩れ無し、「最近使ったシーン」の表示/非表示・順序・重複統合・「続きから」バナーとの重複除外が正しく動作
