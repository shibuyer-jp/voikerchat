...
--   PART B【必須】: 修正コードの動作検証(専用テストユーザーのみ、実機操作を含む)
--
-- 【PART Bは必須、省略不可】api/*.ts にはTypeScript側の自動テストを
-- 追加しない判断をしている(internal-docs/DECISIONS.md 2026-07-26参照)。その判断の
-- 前提は「代わりに実地検証(このPART B)で動作を確認する」ことにあるため、
-- PART Bを省略すると日次リセットのロジックが一切検証されないまま
-- リリースされることになり、この前提が崩れる。シナリオB1(無料ユーザー)は
-- 必須。シナリオB2(Premiumユーザー)のみ、テスト用Premiumアカウントが
-- 無い場合に限り省略可(該当箇所に明記)。
--
...
--
-- 【ストリーク検証(scene_id=1)との干渉について】メッセージ送信は
-- lib/screens/chat_screen.dart で毎回 StreakService.incrementStreak(userId,
-- sceneId) を無条件に呼ぶ副作用があり、この関数は端末の実時刻(1日1回だけ
-- 加算するガード)のみを見る完全にクライアント側・sceneId単位の仕組み
-- (rate_limits.last_reset_utc は一切参照しない)。したがってPART Bの
-- last_reset_utc操作(サーバー側)自体はストリーク判定に影響しないが、
-- 「メッセージを1回送信する」という操作そのものがストリークを1日1回まで
-- 加算してしまう。ストリーク検証(scene_id=1を使用)と同じシーンで
-- PART Bのメッセージ送信を行うと、その日の加算枠を消費し合い、
-- ストリーク検証の期待値(1→2→3)がずれる可能性がある。
-- → PART B(B1・B2)でメッセージを送信する際は、**scene_id=1以外の
-- シーン(例: scene_id=2)** を開くこと。これにより
-- streak_<userId>_<sceneId>_days のキーが完全に分離され、干渉しない。
...
-- ============================================================
-- PART B【必須・省略不可】: 修正コードの動作検証(専用テストユーザーのみ)
-- ============================================================
-- 日付境界を人工的に再現し、実機でアプリを操作して、daily_limitが
-- 正しく基礎値へリセットされることを確認する。**本番ユーザーのデータには
-- 触れない**(冒頭でSETしたテスト用user_idのみが対象)。
--
-- このPARTを省略しないこと。自動テストを追加しない代わりの動作確認が
-- このPARTのみであるため(冒頭の【PART Bは必須、省略不可】参照)。
...
-- scene_id=1【以外】のシーン(例: scene_id=2)を開き、メッセージを1回送信する
-- (またはボイス発話を1回行う)。scene_id=1はストリーク検証で使うため、
-- 同じシーンを使うとストリークの1日1回加算枠を消費し合ってしまう
-- (冒頭の【ストリーク検証(scene_id=1)との干渉について】参照)。
...
-- scene_id=1【以外】のシーン(例: scene_id=2)を開き、メッセージを1回送信する。
-- (B1と同じ理由。Premiumテストアカウントは別ユーザーのため通常は
-- streak_<userId>で既に分離されているが、念のため統一しておく)
