# ARCHIVE.md — Voikerchat 完了事項アーカイブ(追記専用)

このファイルが持つ情報: STATE.mdから退避した完了済み事項の全文。「何が起きたか・どう解決したか」の詳細な記録。STATE.mdには書かず、このファイルにだけ書く。同じ事実を2箇所に書かない。

更新タイミング: STATE.mdの再編・圧縮のたびに、削除対象になった完了済み記述をここへ追記する。担当: CC。

原則: 追記専用。既存の記録は編集・削除しない(DECISIONS.mdと同じ運用)。

---

## 2026-08-13 再編前のSTATE.md全文バックアップ(R4事前バックアップ)

R4(internal-docs再編)着手前、2026-08-13時点のSTATE.md全文(126,151 bytes / 482行)。以下の各セクションは本ファイルの下記セクションへ個別にも整理済みだが、万一の参照漏れに備えて全文もここに残す。
...
> 最終更新: 2026-08-12(続3)(Play Console定期購入レポートのCSVエクスポートで、2026年2月8日〜8月8日の182日間、定期購入が全日程0件であることを確認し、**テスター自身による購入実績はゼロで確定**。未完了項目5「本番環境へのアクセス申請」に残っていた唯一の前提作業(テスター購入実績のPlay Console側確認)が解消し、**申請前の要作業はゼロ**、残るはクローズドテスト完走待ちのみとなった。`internal-docs/PRODUCTION_ACCESS_ANSWERS.md`のPart 1②本文を「確認できていない」から「購入実績はなかった」という確定表現へ4パターンとも差し替え、`internal-docs/PRODUCTION_ACCESS.md`3節も同様に更新。あわせて、同じ確認結果が開発者本人の8/6ライセンステスト購読の実課金なしという結論を別ルートから裏付けたこと、RevenueCat上に同購読の記録が見当たらない件についての新たな仮説(ライセンステスト購入が課金レポート系集計から除外される可能性、[仮説・未確認])をバックログへ追記。詳細はDECISIONS.md 2026-08-12(続3)参照)
> 旧: 2026-08-12(続2)(Play Consoleの「質問のプレビュー」リンクが申請フォームではなくGoogle Playヘルプ記事へのリンクだったと判明し、選択式2問(Part 1①・Part 2③)の選択肢は申請フォームを開くまで確認できない仕様であることが確定。選ぶべき方向性(Part 1①=「難しかった」側、Part 2③=最も低い範囲または下から2番目)をTakatohが確定。同ヘルプ記事からPart 3最終ボタンが「Apply」(申請確定)であること・却下理由例2件の正確な文言を新たに確認。`internal-docs/PRODUCTION_ACCESS_ANSWERS.md`を新設し、記述式6問の日英・標準/短縮版(計24パターン)を申請当日にそのまま貼り付けられる完成形として用意。`internal-docs/PRODUCTION_ACCESS.md`2節・3節・4節・STATE.md未完了項目5に反映。残る前提作業はテスター自身の購入実績をPlay Console側で確認する1点のみ。詳細はDECISIONS.md 2026-08-12(続2)参照)
> 旧: 2026-08-12(続)(`internal-docs/PRODUCTION_ACCESS.md`の製品版アクセス申請前チェックリストを点検し、実態から8箇所ずれていた記載(特典テキスト13→18・データセーフティ申告整合・ポリシーステータス・統計情報の共有)を実画面確認込みで是正。データセーフティ申告は全7項目を実画面で確認し実装との整合を確認、ユーザー作成コンテンツの申告は「収集のみ・共有なし」のまま据え置く判断をTakatohが承認。Play Consoleポリシーステータス「問題は検出されませんでした」・Android Vitals(2026年7月15日〜8月12日)でクラッシュ・ANR記録0件を確認(ただし「クラッシュ報告ゼロ」と断定しない注記を明記)。これにより申請前チェックリスト(PRODUCTION_ACCESS.md 1節)の実画面確認は全項目完了、残るのはクローズドテスト完走待ちのみとなった。PRODUCTION_ACCESS.mdとSTATE.mdの二重管理により項目2の不整合(本書は完了済みだったが同書は「未修正」のまま5日間放置)が発生した教訓も記録。詳細はDECISIONS.md 2026-08-12(続)参照)
> 旧: 2026-08-12(Build 21の配信完了をPlay Console実画面で確認。「21 (1.0.0) / 選択したテスターに公開されました / 8月10日 12:11に公開」[確認済 2026-08-12]。業者依頼「アップデート2〜3回」はBuild 16/18/21の3回で完全充足に確定。クローズドテストは12日目(オプトイン起算 2026-07-30 19:31 JST)、完走見込み2026-08-14で変更なし。統計情報の共有(未完了項目4)は2回目送付を実施し完了へクローズ(3指標とも取得不能なままだが、保証条件は「共有1回以上」であり数値提出そのものではないため、Google側障害告知のスクリーンショットを添えて現況報告した)。あわせてPR #89(返金時のPremium剥奪修正)をマージし本番反映、Vercelデプロイ成功を確認。マージ保留理由だったBuild 21審査中は技術的に無関係と判明(api/revenuecat-webhook.tsはサーバー専用でAABに含まれない)。バックログ「返金時のPremium剥奪の調査」を完了へクローズ(実際の返金イベントでの動作検証は未実施のまま)。派生課題としてusage_logs監査ログ欠落・返金分岐の発火監視をバックログへ新設、技術的負債にtsconfig.json不在によるtsc型チェック形骸化を追記。詳細はDECISIONS.md 2026-08-12参照)
> 旧: 2026-08-10(続4)(RevenueCat RTDN接続が完了。Topic ID `projects/voikerchat/topics/Play-Store-Notifications`、Last received 2026-08-10 4:04 a.m. UTC。バックログ「RevenueCat Android有効化」を全完了へクローズ。事前調査レポートの手順想定が実態と異なっていたため訂正(必要IAMロールはPub/Sub管理者、Pub/Subトピック作成・発行者権限付与はRevenueCatが自動実行)。並行して`api/revenuecat-webhook.ts`のコードレビューで、返金(`CANCELLATION` cancel_reason=`CUSTOMER_SUPPORT`)がPremium剥奪処理から漏れる不具合を発見。2026-06決定(CANCELLATIONでは降格しない)を返金ケースに限り例外化する修正をPR #89として作成したが、Build 21審査中のためマージはBuild 22以降へ保留。RevenueCat上に開発者本人の8/6ライセンステスト購読の記録が見当たらない件も要注意事項として記録(原因[未確認]、実害無し)。詳細はDECISIONS.md 2026-08-10(続4)参照)
...
- **Androidクローズドテスト**: 14日タイマー進行中(起算 2026-07-30 19:31 JST、**2026-08-12時点で12日目**)。Play Console表示「12人のテスターが12日間連続でオプトインしています」[確認済 2026-08-12、Play Console実画面]。完走見込みは従来どおり**2026-08-14前後で変更なし**。**Build 21(versionCode 21)の配信完了を確認**: Play Consoleクローズドテスト(Alphaトラック)のリリース一覧で「21 (1.0.0) / 選択したテスターに公開されました / バージョンコード: 1件 / 8月10日 12:11に公開」を確認した[確認済 2026-08-12、Play Console実画面]。旧: 2026-08-10時点では審査送信までの完了は確認できていたが、テスターへの配信完了は未確認のままだった。統計情報の詳細はSTATE.md未完了項目4参照(2026-08-12に完了へクローズ)。旧: **2026-08-07時点で8日目**。オプトイン不通問題(SNS内蔵ブラウザ起因)は特定・解消済み。詳細はDECISIONS.md 2026-07-30参照。**運用知見**: Play Consoleの「12人」表示は仕様上の固定表示で実数ではなく、実際は毎日16〜19アカウントが稼働(テスター管理業者〈ココナラ経由〉からの回答、2026-08-04)
- **業者依頼「テスト中にアップデートを2〜3回」の充足状況**(2026-08-12更新): **3回すべて配信完了済みで完全充足(Build 16 / 18 / 21)**。Build 16(2026-08-03 10:27 JST配信)・Build 18(2026-08-06 13:46 JST配信)・Build 21(2026-08-10 12:11配信)のいずれも「選択したテスターに公開されました」を確認済み[確認済 2026-08-12、Play Console実画面]。「運用ルール」節の**新AAB配信では14日タイマーはリセットされない**という既存記載と整合しており、Build 21配信もこの例外規定に含まれる(下記「運用ルール」節参照)
...
4. **統計情報の共有(外部サービス保証条件)** ✅ **完了(2026-08-12)**
   - 担当: 人間
   - 検証: 出品者へPlay Console統計を共有済みであること → 2026-08-10(1回目)・2026-08-12(2回目)の2回の送付で充足
   - 期限: 2026-08-14
   - **クローズの根拠**: 保証条件は「共有を1回以上行っていること」であり、統計の数値そのものを提出することではない。Google側の障害により3指標(インストール済みユーザー数・1日のアクティブなデバイス数・DAU)とも取得不能な状態が続いているが、この障害はこちらでコントロールできないものであり、Google公式の障害告知バナーのスクリーンショットを添えて現況を報告済み。完走(8/14見込み)→製品版アクセス申請という日程を踏まえ、データが揃うのを待たず現況報告として送付する判断をした。データが取得可能になった場合は追加送付する旨も業者へ伝達済み[実施済 2026-08-12]
   - **残作業(クローズ後の任意対応、本項目のクローズ条件ではない)**: 統計データの表示が回復した場合は3指標を取得して追加送付する
   - 現況(2026-08-12、2回目送付時点): 3指標とも引き続き取得不能。2026-08-12午前、一時的にPlay Console統計情報が表示可能になったが、スクリーンショット取得を試みた時点で再びGoogle側の障害告知バナー(「現在、一部の1日のアクティブユーザー(DAU)データをご利用いただけません。Googleは現在、この問題の解決に取り組んでいます」)が表示され、3指標とも数値を取得できない状態に戻った[確認済 2026-08-12、Play Console実画面]
   - **運用知見**: Play Consoleの統計情報は、一度表示されるようになっても再び「データを使用できません」に戻ることがある(Google側の障害による)。表示された時点で即座にスクリーンショットを取得すべき
   - 旧: 現況(2026-08-10): 3指標中1指標(インストール済みユーザー数=26、日本23/フィリピン3。ただし2026-08-07の1日分のみで7/30〜8/6分は生成されていない)しか送れておらず、引き続きブロック中として扱っていた[確認済 2026-08-10、Play Console実画面]。テスター管理業者(ココナラ経由)へ現況報告済み(1回目、実施済 2026-08-10)
...
5. **本番環境へのアクセス申請**
   ...
   - 検証: 申請フォームの送信完了
   - 期限: 完走確認後すみやかに
   - 旧前提「『質問のプレビュー』で設問を事前確認しておくこと」は**実施不能と判明(2026-08-12)**したため撤回。Play Consoleの「質問のプレビュー」リンクは申請フォームではなくGoogle Playヘルプ記事へのリンクであり、選択式2問の選択肢は実際に申請フォームを開くまで確認できない仕様と判明した(`internal-docs/PRODUCTION_ACCESS.md`2節参照)
   - **[2026-08-12追記]** `internal-docs/PRODUCTION_ACCESS.md`の申請前チェックリスト(1節、全10項目+4b)は実画面確認込みで全て完了済み
   - **[2026-08-12追記]** 申請当日にそのまま貼り付けられる回答文の完成版(記述式6問、日英・標準/短縮の4パターンずつ)を`internal-docs/PRODUCTION_ACCESS_ANSWERS.md`に用意済み。選択式2問(Part 1①・Part 2③)の選ぶべき方向性もTakatohが確定済み(同ファイル・`internal-docs/PRODUCTION_ACCESS.md`3節参照)
   - **[2026-08-12確定]** Part 1②の残る前提作業だった「テスター自身の購入実績確認」は解消した。Play Console収益レポート(CSVエクスポート)で2026年2月8日〜8月8日の182日間、定期購入が全日程0件であることを確認し、**テスター自身による購入実績はゼロで確定**(詳細はDECISIONS.md 2026-08-12参照)。`internal-docs/PRODUCTION_ACCESS_ANSWERS.md`のPart 1②本文もこの確定事実へ差し替え済み
   - **申請前の要作業はゼロとなった**。残るはクローズドテスト完走(2026-08-14前後見込み)待ちのみ
...
### 製品版アクセス申請前にクリアすべき項目の状況(2026-08-12時点)

Takatohの判断が必要だった項目(年齢設定の不整合〈未完了項目6〉・ライセンステスト登録の確認〈未完了項目16〉・テスターフィードバック再収集の要否〈未完了項目7残作業〉)は2026-08-10までにすべて解消した。

**[2026-08-12更新]** 旧記載では未完了項目4「統計情報の共有(外部サービス保証条件)」が🚧ブロック中として残っていたが、2026-08-12に完了へクローズした(詳細は未完了項目4参照。保証条件は「共有1回以上」であり数値提出そのものではない)。これにより、Takatoh判断待ちだった項目・業者向け契約上の約束事の両方が解消した。

**[2026-08-12(続)更新]** `internal-docs/PRODUCTION_ACCESS.md`のチェックリスト全項目(特典テキスト・データセーフティ申告整合・ポリシーステータス・統計情報の共有を含む)の実画面確認も完了した。**申請前にクリアすべき項目は残っていない**。

残るのは2026-08-14前後のクローズドテスト完走待ちのみである。
...
- **RevenueCat Android有効化** ✅ **完了(2026-08-10、①②③④+RTDN接続すべて完了)**: ①Google Play Consoleで定期購入商品を作成 ✅完了(2026-07-29、`voikerchat_premium_monthly:monthly-autorenew`、174か国) → ②RevenueCatにAndroidアプリ登録+Google Playサービスアカウント認証 ✅完了(2026-08-04)[本人報告、Claude Code未検証]。Google Cloudプロジェクト`voikerchat`(Firebaseと同一)でPlay Android Developer API/Play Developer Reporting API/Cloud Pub/Sub APIを有効化、サービスアカウント`revenuecat@voikerchat.iam.gserviceaccount.com`を作成しPlay Consoleへ招待(アプリ情報閲覧/売上データ/注文と定期購入の管理)。RevenueCatにPlay StoreアプリをApp ID `appf7acdb482b`で追加、Productインポート成功(Published) → ③Offering/Productをマッピング ✅完了(2026-08-04)。Entitlement `Premium`にAttach(App Store版と併存)、Offering `default`の`$rc_monthly`に追加。**Entitlement識別子の大小文字厳密一致は結果的に`Premium`で確定** → ④ビルドに`REVENUECAT_ANDROID_KEY`を渡す ✅完了(2026-08-07確認)。Build 18に投入済みであることを、開発者本人の実購入による課金フロー全体の検証(未完了項目14参照)で確認した → **⑤RTDN(Real-Time Developer Notifications)接続 ✅完了(2026-08-10)**[確認済 2026-08-10、RevenueCatダッシュボード実画面]。Topic ID `projects/voikerchat/topics/Play-Store-Notifications`、ステータス「Connected to Google」(緑チェック)、Last received: 2026-08-10 4:04 a.m. UTC(Play Consoleからのテスト通知を受信)。
  ...
  - **[要注意、2026-08-10]** RevenueCat上に2026-08-06のライセンステスト購読(未完了項目16参照、注文番号`GPA.3366-1618-4848-35600`)が見当たらない。Customers → Active subscribers=0人、Expired=0人、Sandbox=4人(いずれもentitlementsが無く購入イベントも未記録、Sandbox dataトグルをONにして確認済み)[確認済 2026-08-10]。Google Play側では購入が成立しているにもかかわらず、RevenueCat側に該当する購読レコードが見当たらない。**原因は[未確認]**。RTDN未接続だったこと、8/7の実機検証でアプリ削除・再インストールにより匿名user_idが再生成されたこと、ライセンステスト購読が短期間で終了したこと等がcandidateだが特定していない。**実害の評価**: 開発者本人のテスト購読1件が追跡できないのみで、実ユーザー・収益への影響は無い。RTDN接続済みの現在は今後の購入が追跡される見込み。Build 21配信後の実機Paywall確認時に新規購入を行い、RTDN経由のイベント到達を観察する予定
    - **[仮説追加、2026-08-12、未確認]** Play Consoleの定期購入レポート(収益レポート)をCSVエクスポートして確認したところ、当該ライセンステスト購読(2026-08-06)は収益レポートにも一切現れていなかった(2026年2月8日〜8月8日の182日間、定期購入は全日程0件)。ライセンステスト購入は実課金を伴わないため収益レポートの集計から除外されるという整理と整合しており、未完了項目16の結論(実課金なし)を別ルートから裏付ける形になった。この「ライセンステスト購入は課金レポート系の集計から除外される」という共通の仕組みが、RevenueCat側に同購読の記録が見当たらない件の原因である可能性がある。ただしRevenueCatはSandboxデータを別途表示する機能を持つため断定はできない([仮説]、原因調査の手がかりとして記録)。詳細はDECISIONS.md 2026-08-12参照
- **返金時のPremium剥奪の調査** ✅ **完了(2026-08-12、PR #89マージ・本番反映済み)**: RevenueCatには独立した`REFUND`イベント種別が存在せず、返金は`CANCELLATION`イベントの`cancel_reason='CUSTOMER_SUPPORT'`として届くことをRevenueCat公式ドキュメント「Event Types and Fields」で確認した(2026-08-10)。現状の`api/revenuecat-webhook.ts`は`REVOKE_EVENT_TYPES=EXPIRATION`のみで`CANCELLATION`を一律`action='none'`として素通りしており、**返金が成立してもPremiumが剥奪されない不具合**があった。`EXPIRATION`に頼ることもできない(返金後も自動更新設定が有効なままのことがあり、その場合はRENEWALが先に発火しEXPIRATIONが届かない)。2026-06の既存決定「CANCELLATIONでは降格しない。EXPIRATIONのみ降格」は自発的解約(UNSUBSCRIBE)については引き続き正しく、本件は**その決定の全面撤回ではなく返金ケースに限定した例外追加**。修正はPR #89(`fix/refund-cancellation-premium-revoke-20260810`)として作成し、マージ前レビュー(項目1〜6、DECISIONS.md 2026-08-12参照)を経て2026-08-12にマージ、Vercel経由で本番反映済み(デプロイ成功確認済み)。当初の保留理由「Android Build 21審査中」は技術的に無関係だったと判明(`api/revenuecat-webhook.ts`はサーバーサイド専用でAABに含まれない)。**実際の返金イベントによる動作検証は未実施**(検証可能な生きた購読が現時点で存在しないため)。派生課題は下記2件をバックログへ新設。詳細はDECISIONS.md 2026-08-10(続4)・2026-08-12参照。担当: CC(実装・レビュー・マージ)完了
- **返金分岐の発火監視**(優先度: 中、一般公開後の運用項目、2026-08-12追加): RevenueCatの`cancel_reason='CUSTOMER_SUPPORT'`は「カスタマーサポート経由の解約」であり、返金を伴わないケースが理論上ありうる[未確認]。その場合、期間内の権利があるユーザーからPremiumを即時剥奪してしまう。実害の可能性は低く現設計(PR #89のホワイトリスト方式)を維持するが、公開後にこの分岐が発火した際は「Premiumが急に使えなくなった」という問い合わせが来ていないか注視する。担当: 人間。期限: 一般公開後
- **revenuecat-webhookの監査ログ欠落**(優先度: 中、製品版公開前に解消したい、2026-08-12追加): `api/revenuecat-webhook.ts`は`rate_limits`のみ更新し`usage_logs`へ書き込まないため、返金による剥奪の痕跡が`console.log`(Vercelファンクションログ、保持期間が短い)にしか残らない。`api/premium-sync.ts`の`logSyncAttempt()`(event=`premium_sync`)に相当する仕組みをwebhook側にも追加すべき。担当: CC。期限: 製品版一般公開前
...
- Build 16(2026-08-03配信)・Build 18(2026-08-06配信)・Build 21(2026-08-10 12:11配信)のリリースのみ例外。**新AAB配信では14日タイマーはリセットされない**ため、今後もビルド配信自体は可能
...
- ~~api/ に tsconfig.json / lint 設定が存在しない~~ → **tsconfig.json新設・型チェックCI導入で解消(2026-08-13、R3)**
  ...
  - **[2026-08-13新設]** `api/tsconfig.json`(`strict: false`、`target: ES2020`、`include: ["**/*.ts"]`、Flutter側と混ざらないよう`api/`配下に限定)、`package.json`に`scripts.typecheck`(`tsc --noEmit -p api/tsconfig.json`)、`.github/workflows/api-typecheck.yml`(Flutterジョブとは分離、`pull_request`/`push`で実行)を追加。初回は`continue-on-error: true`で導入したが、CI実環境(Ubuntu runner、Node 18)で実際に0エラーで通ることを確認できたため、同PR内でこのフラグを外しブロッキング化した。詳細はDECISIONS.md 2026-08-13参照
- **`npm install`時に`@supabase/supabase-js`等がNode 22以上を要求する`EBADENGINE`警告**(2026-08-13、api-typecheck CI初回実行で発見): `package.json`の`engines.node`は`>=18.x`のままで、CIも`actions/setup-node`でNode 18を使用しているが、依存パッケージ側は既にNode 22+を要求している。現状はビルド自体は失敗しないため実害なし。将来Node 18のサポートが実際に切れるタイミング、または依存関係を更新するタイミングで`engines.node`とCIのNode版を22以上へ揃える対応が必要。担当: 未定。期限: 未定
...
- ~~`Documents/voikerchat`(別チェックアウト、2026-06-29時点の古い状態)の git remote URL に GitHub PAT が平文で埋め込まれている(2026-08-01にCCが発見)~~ → **判断のうえ据え置き(2026-08-13)**。該当チェックアウトはサルベージ不要と確認のうえ削除済み、ローカル全13リポジトリ・credential関連ファイルを走査したが他に埋め込みなしと確認済み(CC実施)。**PATの新規発行・差し替え・旧PATのrevokeは実施しないとTakatohが判断**: 平文の露出範囲が本人PC/OneDrive限定で第三者共有経路がなく漏洩形跡ゼロである一方、8/14の製品版アクセス申請を控えた時期に自動化基盤(CC・chat-Claude双方が依存)を触るリスクの方が高いと評価したため。代替担保としてOneDriveごみ箱・バージョン履歴の消去とGitHub Security logの定期確認(いずれも人間)を実施。**再検討トリガー: 将来PATを第三者と共有する場面が生じた時点**。詳細はDECISIONS.md 2026-08-13参照
...
## 完了済み未完了項目(元STATE.md「未完了項目」節より、番号を維持したまま移動)
...
5. **本番環境へのアクセス申請** ✅ 申請提出完了(2026-08-14 09:04 JST)。審査結果はSTATE.md「進行中」参照
...
12. **iOS Sandbox課金検証** ✅ クローズ(2026-08-14)
    - **✅ クローズ(2026-08-14)**。残作業ゼロ。項目12にぶら下がっていた
      3論点はいずれも解消済みであり、STATE.mdに未完了として残っていたのは
      記録の取り残しだった:
      1. 価格の食い違い(App Store ¥2,000 vs Google Play JPY 2,120) →
         解消済(2026-08-07)。Google Play側を JPY 1,818 / PHP 713 へ値下げして
         揃えた(詳細はARCHIVE.md該当項目参照)
      2. 購入シートの「UNRATED」表示 → 実質解消(2026-08-10)。項目6で
         App Store Connectの年齢制限が13+であることを実画面確認し、
         4箇所の実態を確定表としてクローズ済み。TestFlight/Sandbox環境
         特有の表示だった可能性が高い【推測、未検証】
      3. PR #53(購入直後のシーン解除)の実機検証 → 検証不能と確定
         (2026-08-07)。TestFlight環境では未購入状態を作れないため、
         コードレビューによる確認に留める方針をTakatohが決定済み。
         リスクは公開後の監視で代替する
...
13. **iOSの公開タイミング判断** ✅ **完了(2026-08-27)**。両OS同時公開を 2026-08-27 に達成
    - 状態: 1.0.0+20のPending Developer Releaseはキャンセル済み。1.0.0+22は
      2026-08-19にGuideline 2.3.10でリジェクトされ、2026-08-20にメタデータ
      のみ再提出済み(Waiting for Review)。**公開対象は +22**
    ...
    - **完了(2026-08-27)**: Android を先に一般公開(製品版リリース22が下書きの
      まま滞留していた問題を是正して公開。DECISIONS.md 2026-08-27(Android)参照)、
      実画面確認後に iOS も「このバージョンをリリース」を実行し「配信準備完了」へ
      到達。撤退期限 2026-09-05 は到来前に達成したため失効した。
      iOS のストア反映確認は TRIGGERS.md「Android と iOS の両方が実際に
      公開されたとき」で継続。
...
21. **iOS 1.0.0+22 メタデータ再審査の結果確認** ✅ **完了(2026-08-27、承認)**
    - 却下時はResolution Centerの指摘内容を`internal-docs/DECISIONS.md`へ記録する。
    - 担当: 人間。期限: 撤退期限 2026-09-05
    - **完了(2026-08-27)**: 2026-08-20 提出のメタデータのみ再提出が承認された
      (Submission ID a9d47ca9-399f-4211-b80e-1a4e4505d9d2)。同日 iOS 1.0.0(22) を
      リリースし一般公開に到達。iOS は 1.0.0 リリースまでに計5回の App Review
      提出を要した(過去のリジェクト理由: 3.1.2 EULAリンク / 5.1.1(iv) マイク権限 /
      5.1.1(i)・5.1.2(i) AI送信同意 / 2.3.10 Google Play言及)。
      詳細は DECISIONS.md 2026-08-27(iOS)参照。


## 製品版アクセス申請前にクリアすべき項目の状況(元STATE.md、2026-08-12時点の記録)
...
## 完了済みバックログ・改善候補項目(元STATE.md「バックログ」「改善候補」節より)
...
## STATE.mdヘッダ「最終更新」旧エントリ(2026-07-27〜2026-08-12(続2)、21件)
...
## ROADMAP.md 全文(2026-08-07新設・2026-08-13廃止、STATE.mdへ一本化)
...
1. **統計情報の共有**(STATE.md未完了項目4) — インストール数のみ8/7分が生成済み、
   ...
   **[要注記]** 本項目は「外部サービス保証条件」(テスター管理業者への契約上の約束)であり、
   ...
   とは性質が異なる点に注意
2. **ストア掲載情報の見直し(ASO)**(2026-08-10追記、`internal-docs/GROWTH_PLAN.md`参照) —
   PR #20のストアコピペ反映を含む。アプリの流入の大半はストア内検索であり、
   SNSより先に効くと判断し区分3から格上げした。フィリピン人学習者が実際に検索する語
   (Japanese / Nihongo / Tagalog 等)がタイトル・短い説明に入っているかを確認する。
   `internal-docs/Competitor-Insights.md` 2026-07(初版)実装ロードマップ案4と同一の作業
3. **公開日のレビュー依頼準備**(2026-08-10追記、`internal-docs/GROWTH_PLAN.md`参照) —
   配偶者経由の実テスター(クローズドテストに参加した16名)へ、公開日にレビュー投稿を
   依頼できるよう事前に声をかけておく。公開日確定後に実施
4. `PRODUCTION_ACCESS.md`のチェックリスト全項目の最終確認
5. 本番環境へのアクセス申請フォームの送信(区分1参照)
...
- **年額プランの追加**[優先: 高](2026-08-10追記、`internal-docs/Competitor-Insights.md` 2026-08-10追補参照)
  - 内容: RevenueCatに`$rc_annual`を追加、両ストアで年額商品を登録、Paywallを2プラン対応にする
  - 根拠: 競合(スピークバディ・ELSA Speak)は年額契約が主力(月換算2,316〜2,367円)。
    Voikerchatは月額(1,818円)では競合の半額程度と競争力があるが、年額プランが
    存在しないため比較の土俵に乗れていない。年額プランの不在は構造的な弱点
  - 着手時期: 実装コストが小さい割にLTVとキャッシュフローへの効果が大きいため、公開直後に着手

- **学習スコアの可視化**[優先: 高](2026-08-10追記、`internal-docs/Competitor-Insights.md` 2026-08-10追補参照)
  - 内容: ELSAは全レッスン結果を自動スコア化し5項目+総合、さらに直近10レッスン分を
    毎回提示する。Voikerchatは`usage_logs`があり集計基盤は存在する
  - **第一段階は追加のAI呼び出しなしで実装できる範囲に限定**(会話数・連続日数・
    使用語彙数)。トークンコストゼロで効果を狙う
  - 着手時期: 公開直後

- **無料トライアル(全機能開放)**[優先: 中](2026-08-10追記、`internal-docs/Competitor-Insights.md` 2026-08-10追補参照)
  - 内容: 現状は無料10回/日が恒久的だが、Premium限定シーンを体験する機会がなく
    課金判断の材料が乏しい
  - **トークンコスト増に注意**。7日ではなく初回3日間など短めから試し、実績を見て
    延長を判断する
  - 着手時期: 公開後、運用が落ち着いてから

- **ユーザー定義シーン**[優先: 中](2026-08-10追記、`internal-docs/Competitor-Insights.md` 2026-08-10追補参照)
  - 内容: 既存のAI会話基盤にシーン設定を差し込むだけで技術的には成立し、18シーンの
    制約を外せる
  - **懸念2点**: ①プロンプト長がキャッシュ効率を下げコスト増 ②ユーザー入力を
    直接プロンプトに入れるため、13歳以上向けアプリとして不適切なシーン設定への
    対策が必須
  - 着手時期: 公開後、運用が落ち着いてから

- **在日フィリピン人向けの実用シーン追加**[優先: 中](2026-08-10追記、`internal-docs/Competitor-Insights.md` 2026-08-10追補参照)
  - 内容: 役所手続き、病院の受付・受診、職場の報連相、ゴミ出し、子どもの学校からの連絡、
    介護現場の申し送り
  - **7月版で既にニッチとして定義済みの「介護/特定技能の実務シーン」の具体化**であり、
    新規方針ではなく継続(`internal-docs/Competitor-Insights.md` 2026-07(初版)「Voikerchatの立ち位置」参照)
  - 着手時期: 公開後
