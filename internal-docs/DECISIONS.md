...
### 2026-08-05 無料枠の単位・数値変更

**背景**
テスターから「5問質問しただけで1日の上限に達する」と報告 [確認済 2026-08-05 / 妻経由]。調査の結果、実装は仕様通りでありバグではないと判明 [確認済 2026-08-05 / コード調査]。
- 無料枠は「メッセージ単位(＝API呼び出し1回)」でカウントされている
- 根拠: `api/_constants.ts:8-13`、`api/chat.ts:99`/`237-239`/`243`
- 会話セッション識別子は存在するが`rate_limits`と未配線のため、現状のデータモデルでは「会話単位」カウントは成立しない
- 社内でも「会話」と「呼び出し」を同義的に使用してきた形跡があり(`recordConversation`等)、用語の不整合による期待値のズレが実態

**決定**
問題は単位ではなく数値。5メッセージでは1つの会話すら完結せず、アプリの価値が伝わる前に枯渇する。以下に変更する。

| 定数 | 現行 | 変更後 |
|---|---|---|
| FREE_DAILY_LIMIT | 5 | 10 |
| FREE_DAILY_CAP | 10 | 20 |
| AD_BONUS | 5 | 5(据置) |
| PREMIUM_DAILY_LIMIT | 50 | 50(据置) |

Premium を据え置く理由: 差別化は回数比(1:5)に加え、アニメシーン5種・広告なし・学習統計という非数量的要素が担う。Premium を引き上げると原価率が悪化し、差別化効果も薄い。

**採算根拠**
- 単価 [確認済 2026-08-05 / Anthropic公式]: Claude Haiku 4.5 = 入力 $1 / 出力 $5 per 1M tokens
- ストア手数料15%(小規模事業者プログラム) → Premium 純収入 $11.04/月

| 項目 | 値 | 状態 |
|---|---|---|
| 1メッセージ原価(標準想定 入力2000/出力200 tok) | $0.0030 | [未確認] トークン数は想定値 |
| 無料1人あたり月間原価(実勢) | $0.324 | [未確認] |
| 広告収入(eCPM $3・月6本) | $0.018 | [未確認] |
| 無料1人あたり月間損益 | -$0.306 | [未確認] |
| Premium 1人あたり粗利(実勢) | +$10.17 | [未確認] |
| **損益分岐 課金率** | **2.9%** | [未確認] |

感度: トークン量が悲観値(入力3500/出力300)だと分岐点5.2%となり採算が成立しない。ヘビーユーザー比率の変動より影響が大きい。

**公開ブロッカー(一般公開前に必須・未実施のまま公開しないこと)**

1. **実トークン数の計測**
   システム指示・シーン定義・会話履歴・出力の実際のトークン数を測定する。分岐点が2.9%か5.2%かはこの値で決まる。`usage_logs.input_tokens`/`output_tokens`が既に存在するため、ベータ期間の実データから集計可能。

2. **原価削減策の実装**
   - プロンプトキャッシュ(固定部の入力料金が1割になる): 単価-24%
   - 会話履歴を直近6往復に制限: 単価-17%
   - 両方適用で単価-41%、分岐点1.6%=安全圏
   ※ 会話体験は損なわない(直近のやり取りは保持されるため)

クローズドベータ中(テスター61名)は最悪でも月$110のため、上記2点の完了前でもベータ継続は可能。

**未確認事項**
- [未確認] 「1日5会話」という仕様の一次情報源(PRD/チケット)の有無
- [未確認] 本番デプロイ済みAPI(Vercel)のコードがmainと一致しているか
- [未確認] `ad-reward.ts`のリセット基準(UTC暦日)と`rate_limits`の基準(前回リセットから24時間のローリング)が不一致。別途調査が必要
