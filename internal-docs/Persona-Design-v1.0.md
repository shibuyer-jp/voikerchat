...
## 基本8シーン + アニメ風5シーン + 実用プレミアム5シーン（id14〜18、T-34）

> **2026-08-04 注記**: 本セクションの詳細ペルソナ定義（グループ1・2）はv1.0作成時（2026-06-19、13シーン体制）の内容のまま。2026-07-29以降のT-34でid14〜18の実用プレミアム5シーンが追加され、実装は現在合計**18シーン**（詳細は下記グループ3、および`internal-docs/tasks/T-34_premium-pro-scenes.md`・`lib/services/scene_service.dart:160-204`参照）。
...
### グループ 3: 実用プレミアム5シーン（id14〜18、T-34）

「日本で働く外国人」視点の就労・生活実務シーン。ターゲット(フィリピン人学習者)の実需=就労日本語に直結させる目的で2026-07-29以降に追加。既存の病院シーン(id 5、患者視点)とは異なり、id14/15は介護・医療現場の**スタッフ**視点。詳細な性格特徴・口調例・System Promptはグループ1・2と異なりこの設計書には未収録で、`api/chat.ts`のシステムプロンプト（`internal-docs/tasks/T-34_premium-pro-scenes.md`記載の方針に従い実装）が正となる。id・シーン名・キャラクター名・推奨レベルは`lib/services/scene_service.dart:160-204`参照。

| id | シーン名 | キャラクター | 内容 | 推奨レベル |
|----|---------|--------|------|-----------|
| 14 | 介護のしごと | Haruko | 介護施設での声かけ・体調確認 | Intermediate |
| 15 | 医療スタッフ | Mori | 医療現場での申し送り・報告 | Advanced |
| 16 | 面接 | Sato | 特定技能・就労面接の練習 | Intermediate |
| 17 | 役所・手続き | Mizuki | 在留・住民票などの窓口手続き | Intermediate |
| 18 | 職場の敬語 | Tanaka | 報連相・依頼・謝罪の敬語 | Advanced |
...
- 計算例(v1.0時点、13シーン基準): 13シーン × 約330トークン(平均) + 共通581 ≈ 4,043トークン
- **2026-08-04更新(18シーン基準)**: 18シーン × 約330トークン(平均) + 共通581 ≈ 6,521トークン。id14〜18の5シーン分は本設計書に実測値の記載がないため、既存13シーンの平均(約330トークン/シーン)を外挿した概算値であり、実測ではない点に注意
